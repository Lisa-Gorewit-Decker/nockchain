...
- `TerminalNpoPolynomialPaddingQuotientProof`: the quotient-backed value-column
  ...
  bit_tail(zeta)` for non-base lanes. Hidden Tip5 padding and zero constraints
  are Merkle-direction aware: for Merkle rows, the selector is
  `normal_hidden + mmcs_bit * (swapped_hidden - normal_hidden)`, where
  `normal_hidden` and `swapped_hidden` are verifier-derived after multiplying
  in the relevant row-mode gates. New-start Merkle rows zero only hidden lanes
  sourced from the prior running state (`bus_limb < 5` after the
  `mmcs_bit`-selected mapping); serialized sibling lanes `5..9` are allowed,
  and Merkle capacity lanes `10..15` are always zeroed when present. The same
  quotient also enforces recompose constraints forcing non-base input tails to
  zero and `output_basis_i = input_i_base` under the combined
  `is_recompose + is_recompose_coeff` selector. The quotient domain is twice
  the value-column trace domain so the degree-3 booleanity/dynamic-selector
  relations have low-degree quotients. Its standalone prelude commitment vector
  is exactly the witness-value FRI root; the quotient root is not prelude
  material because it
...
- `TerminalNpoTip5PackedLookupTrace` profile and data source: the next
  ...
  packed row contains round-0 `IN`, split `(b,c)` byte-pair columns, guard
  inverses, and `OUT`; later round `IN` values are aliases of the previous
  round's `OUT`, so table rows and repeated carried-input columns stay out of
  the packed matrix. The focused regression checks packed rows against the
  existing lookup trace and terminal-derived Tip5 IO. On the full
  `ai-pow-zk` PROD composite relation, this compact shape has width `436`,
...
- `TerminalNpoTip5PackedLookupAirAlgebraQuotientProof`: the first committed
  ...
  round links, split bytes, and power lanes. The full PROD diagnostic now
  measures this standalone checkpoint at `129,471` bytes / `126.4 KiB`,
  compact FRI `120,507` bytes, full-trace zeta openings `8,756` bytes, prove
  `19.875s`, and verify `10.402s`. This is a quotient-runtime improvement, not a
...
- `TerminalNpoTip5PackedLookupLogupQuotientProof`: the second committed proof
  ...
  multiplicities. The full PROD diagnostic now measures this standalone
  checkpoint at `155,974` bytes / `152.3 KiB`, compact FRI `144,547` bytes,
  full-trace zeta openings `8,709` bytes, table opening `21` bytes,
  accumulator openings `1,848` bytes, prove `22.284s`, and verify `11.055s`.
  This closes the standalone byte-table binding, but it is still above the
  relaxed `~150 KiB` target and is not appendable to the merged value-bridge
  body.
...
- `TerminalNpoTip5LookupLogupQuotientProof`: a committed low-degree split
  ...
  selected-vs-trace LogUp multiset compare masked IO values. A prior polynomial
  native / Rayon production-candidate checkpoint was recorded as `99,289`
  ...
  verifier algebra. An older 2026-06-05 integrated group-size sweep
...
- `TerminalNpoPolynomialFriResidualZeroRecomposeValueBridgeProof`: a combined
  ...
  residual-zero composition polynomial, the recompose residual quotient, and a
  shared padding+value-bridge quotient in one terminal FRI proof. The current
  full-composite `ai-pow-zk` diagnostic serializes this merged proof with the
  terminal prelude and primitive row-product proof as a `151,448` byte body:
  `240` bytes of prelude, `57,501` bytes of primitive proof, and `93,707`
  bytes of merged NPO proof with `91,501` bytes of compact FRI payload. It
  verifies over the actual composite L1 verifier relation. After prover-work
  reuse, prepared merged NPO data, prelude-checked prover entry points, and
  batched-LDE padding/value-bridge quotients committed together on the shared
  quotient domain, post-prelude serial proof-body construction is `14.914s`:
  `8.372s` primitive R1CS plus `6.541s` merged NPO proof. The full diagnostic
  wall is still `108.606s` because setup and verification are outside that
  proof-body timer. The value bridge is Merkle-direction aware: it uses the
  committed `mmcs_bit` to select the bus-limb expression that feeds each Tip5
  trace lane. The padding quotient now proves `mmcs_bit` booleanity,
  zero-when-absent behavior, value-padding consistency, dynamic hidden Tip5
  padding, new-start zero constraints, capacity-zero constraints, and
  recompose value semantics. This checkpoint still lacks the internal Tip5
...
Earlier checkpoints still used `BatchStarkProver` for the recursive outer
proof, with `build_terminal_witness` packaging the verifier-circuit execution
before that proof step. The production terminal target consumes the same circuit
and witness bundle while replacing the oversized batch-STARK proof format.
...
| component | bytes |
|---|---:|
| primitive R1CS row-product proof | 22,631 |
...
| compact production proof body | 85,726 |
| compact production certificate | 85,948 |

The latest release-profile measurement is `prove=1.492 s, verify=1.181 s` for
...
measured run produced an `85,948` byte / `83.9 KiB` serialized certificate,
...
| FRI-native merged residual-zero/recompose/padding/value-bridge candidate, full AI-PoW composite measurement, verified with primitive/prelude body `151,448` bytes | 93,707 | - | 91,501 | 6.541 s post-prelude prepared | 13.525 s |
...
The merged residual-zero/recompose/padding/value-bridge full-composite
measurement confirms that the next NPO identity layer can still fit near the
relaxed byte target and that post-prelude body construction can fit the relaxed
time target. The merged proof body is `93,707` bytes with `91,501` bytes of
compact FRI material; the complete diagnostic body including prelude and
primitive proof is `151,448` bytes. Post-prelude serial proving takes
`14.914s`, split between `8.372s` primitive R1CS row product and `6.541s`
prepared merged NPO proof. The bridge binds Merkle direction by selecting the
value expression with the committed `mmcs_bit`, and the padding quotient proves
the `mmcs_bit` zero/boolean/padding constraints plus dynamic hidden-lane,
new-start, capacity-zero, and recompose value semantics. The remaining
production composition gap is the internal Tip5 lookup/AIR/LogUp relation.
...
`96,017` bytes / `93.8 KiB` and proves in `9.918s` after prepared
selected+lookup reuse, a batched-LDE AIR quotient builder, and a prover-only
folded-AIR evaluator cleanup. That checkpoint includes the Tip5 AIR, byte
LogUp, raw-trace support bridge,
...
composite diagnostic reached `14.435s` primitive proving plus `2.340s` merged
...
full PROD relation at `129,471` bytes with `19.875s` proving. The packed
byte-table LogUp checkpoint also verifies on the full PROD relation at
`155,974` bytes with `22.284s` proving, closing the standalone table-membership
...
variables, and `489,990` entries. The `57,501` byte primitive proof is
dominated by its `55,025` byte assignment evaluation proof, not by sumcheck
rounds: row-product rounds are `1,327` bytes and matrix-sumcheck rounds are
`1,088` bytes. Assignment fold-round openings are `53,586` bytes, of which
`48,462` bytes are the `1,211` Merkle frontier nodes needed to authenticate 15
query-derived fold paths across 18 assignment fold layers. Precomputing
equality-polynomial tables for sparse R1CS evaluation did not materially change
...
prover entry points then reduced post-prelude primitive proving to `9.232s` in
...
checks Tip5 chain-start zero lanes and Merkle capacity-zero lanes under dynamic
`mmcs_bit`-selected hidden selectors, plus recompose value-column semantics
under verifier-derived row selectors. The optimized
...
evaluation.
The padding quotient is now also folded into the full-composite merged
residual-zero/recompose/value-bridge proof. In the focused component path, the
compressed FRI payload remains small; in the production-scale merged path, the
padding and value-bridge quotients share one quotient-domain commitment and one
opening batch rather than adding a standalone FRI proof.
...
verifier-accepted inside the merged residual-zero/recompose/padding/value-bridge proof.
It uses the committed `mmcs_bit` as the direction selector from callsite
bus-limb coordinates into Tip5 trace lanes. The merged padding quotient now
proves the paired padding/boolean/present constraints that make `mmcs_bit` zero
when absent and boolean when present, including dynamic hidden selectors for
Merkle rows. The remaining composition obligation is the internal Tip5
lookup/AIR/LogUp relation and its shared commitment/transcript binding.
...
| Production certificate is about 100 KiB | The current real Tip5 L0 verifier-circuit release measurement produced `85,948` bytes / `83.9 KiB` and passed the hard size assertion. The previous polynomial NPO production body measured `226,248` bytes / `220.9 KiB` and remains diagnostic, not the production wire path. | satisfied by the current exhaustive-NPO production measurement |
...
| Supported NPO rows cannot hide invalid sampled rows | Production does not sample supported NPO validity. `TerminalNpoExhaustiveProof` opens every verifier-derived supported Tip5/recompose callsite against the same assignment oracle used by primitive R1CS, checks hidden Tip5 lanes, MMCS direction-bit booleanity, row modes, predecessor-chain semantics, and recompose rows deterministically, and rejects missing/tampered assignment-witness multiproof material or hidden-input payloads. | satisfied for supported NPO row validity |
| Supported NPO/table rows are polynomialized into a final proximity backend | The polynomial/proximity backend exists and remains tested as a diagnostic and future hardening path, but it is not the production wire body because the complete two-subproof polynomial payload measured `226,248` bytes / `220.9 KiB`. | implemented as diagnostic; not the active production payload |
| Full terminal proof has a source-backed soundness calculation | The theorem below states the active production terminal argument as primitive sparse-R1CS row-product sumcheck plus exhaustive supported-NPO row checking under one assignment-root prelude, with 5-round Tip5 Merkle binding and `log_blowup=4`, `num_queries=15`, `query_pow_bits=0` for the primitive row-product/assignment-evaluation proximity checks. The 60-bit production floor is not backed by terminal query PoW. | satisfied as a source-backed conditional theorem |
| Zero-knowledge or witness hiding for recursive-verifier witness values | Current production does not serialize the full witness, but exhaustive supported-NPO openings reveal selected recursive-verifier witness material, including hidden Tip5 input lanes needed to recompute hidden-input rows. | incomplete if ZK is required |
...
- The current production proof is a sumcheck-backed primitive sparse-R1CS
  ...
  small and verified, and the merged residual-zero/recompose/padding/value-bridge proof
  now verifies at `151,448` bytes including prelude and primitive proof. This
  is still diagnostic: post-prelude proving fits the relaxed time gate, but the
  full diagnostic wall still includes setup cost, and the path still needs the
...
- The sparse-R1CS matrix-vector sumcheck is an evaluation subclaim. It avoids
  ...
  sumcheck for primitive R1CS rows. Supported NPO/table rows are covered by
  exhaustive row checking against the same assignment oracle rather than by the
  primitive sparse-R1CS proof.
- The terminal oracle Merkle layer is binding to opened values under the
  ...
  serialization; supported NPO rows are handled by known-index exhaustive
  assignment-witness openings plus verifier-selected hidden Tip5 input payloads.
...
- The exhaustive NPO production verifier avoids a sampled-row gap by checking
  every supported row. Its proof remains linear in the supported-NPO
  witness-opening surface, but the current measurement is still under the byte
  and time targets.
...
- The sampled primitive and supported-NPO local proofs remain useful regression
  ...
  argument plus exhaustive supported-NPO row checks under the same assignment
  root.
- The quadratic residual oracle and fold proofs remain useful checkpoints for
  ...
  carried by exhaustive row checking.
...
- The NPO validity fold proof gives supported NPO rows a matching
  ...
  after a full assignment check. Production now relies on exhaustive
  supported-NPO row checking instead; polynomialized NPO proofs remain
  diagnostics and future hardening work.
...
- The typed exhaustive-Merkle terminal production path is measured at
  85,948 bytes, or 83.9 KiB, with 60 pure-query bits and exhaustive
  ...
  The integrated polynomialized NPO/proximity backend preserves a lower-leakage
  direction for future hardening, but it is not the current public production
  path because it misses the byte/time target.
- Completion status: the public production certificate uses primitive
  row-product plus exhaustive supported-NPO checking at the canonical 60
  pure-query-bit terminal profile. The current real release measurement is
  `85,948` bytes / `83.9 KiB` with `prove=1.492s`, satisfying the hard target.
  The written theorem and source-backed cryptographic review below document the
  conditional soundness model, subject to the stated FRI, Fiat-Shamir, and Tip5
  assumptions.
...
- The production path in `crates/plonky3-recursion/recursion/src/terminal.rs`
  builds `TerminalProductionProof` from one assignment root, then verifies the
  primitive row-product proof and, for supported NPO rows, an exhaustive NPO
  proof against the same assignment commitment.
...
4. the verifier computes public-value digests, relation digests, supported-NPO
   callsites, hidden-lane rules, fixed tables, and verifier-derived witness
   index sets honestly from `R`.
...
- a false supported-NPO row passing the deterministic exhaustive Tip5/recompose
  row verifier against assignment-oracle openings;
...
1. Prelude binding. `TerminalCertificate` binds the protocol id, production
   ...
   relation profile, public-values digest, assignment root, zero query-PoW
   nonce, and proximity schedule before any backend challenge is sampled. This matches
...
3. Supported NPO rows. `TerminalNpoExhaustiveProof` opens the exact
   verifier-derived witness IDs for every supported Tip5/recompose callsite
   against the same assignment commitment. The verifier recomputes hidden Tip5
   lane placement, row modes, MMCS direction-bit booleanity, predecessor-chain
   carry lanes, normal Tip5 rows, Merkle Tip5 rows, and recompose rows. A stale
   or independently substituted NPO witness therefore has to either break the
   assignment Merkle opening or satisfy the actual supported-NPO row semantics.
4. FRI restoration. `TerminalCompressedFriProof` is soundness-neutral
...
- The production proof is not zero-knowledge. It is smaller than full witness or
  full-witness serialization, but exhaustive supported-NPO openings reveal
  selected recursive-verifier witness material, including hidden Tip5 input
  lanes needed to recompute hidden-input rows.
...
- production proof body uses primitive row-product plus exhaustive
  supported-NPO checking under one assignment-root prelude;
- focused tests tamper production proof bodies, noncanonical production
  parameters, stale preludes, missing roots, missing exhaustive assignment
  openings, hidden Tip5 input tampering, assignment-witness multiproof
  tampering, and recompose-row witness tampering;
- the current real production measurement is `85,948` bytes / `83.9 KiB` with
  `prove=1.492s` and `verify=1.181s` in release mode for the full
  primitive+exhaustive-NPO terminal proof body;
...
1. Keep the typed compact production certificate as the only public terminal
   ...
   hashing, primitive sparse-R1CS row-product proof, and exhaustive
   supported-NPO proof.
...
3. Measure proof bytes, prove time, verify time, and the explicit soundness
   accounting after every major relation/proof-shape change. The current real
   measurement is `85,948` bytes / `83.9 KiB` with `prove=1.492s` and
   `verify=1.181s` in release mode. The retired polynomial NPO production
   candidate measured `226,248` bytes / `220.9 KiB` and remains a diagnostic
   hardening track, not the active production wire path.
