...
- `TerminalNpoTip5LookupIoSupportBridgeQuotientProof`: a single-quotient
  ...
  proof, measures `96,994` bytes / `94.7 KiB` at the production
  `log_blowup=4, num_queries=15, query_pow_bits=0` profile, and rejects both a
  nonzero lookup-table-row IO value and a stale permutation-row lookup IO
  value. This is the current preferred boundary-check candidate because it
  replaces the separate zero-support and bridge proofs with one FRI proof; it
...
- `TerminalNpoTip5LookupAirAlgebraQuotientProof`: a quotient-backed checkpoint
  ...
  tampered x2 auxiliaries. This closes a real internal-AIR algebra/support
  checkpoint, but it is too large to combine naively with the current terminal
  backend and still excludes the global LogUp byte-table argument.
...
| component | bytes |
|---|---:|
| primitive R1CS row-product proof | 21,321 |
| exhaustive NPO proof | 62,787 |
| exhaustive NPO hidden Tip5 input bytes | 17,402 |
| exhaustive NPO known-index witness multiproof | 45,385 |
| exhaustive NPO sparse witness basis coefficients | 1,521 |
| compact production proof body | 85,562 |
| compact production certificate | 85,785 |

The debug-profile measurement is `prove=4.952 s, verify=3.214 s` for the
...
its extension-field witness basis values omit zero coefficients under a
verifier-checked coefficient bitset before Merkle-root reconstruction. The
...
|---|---:|---:|---:|---:|---:|
| full-table NPO FRI opening proof, 186 field columns / 372 basis columns | 99,647 | 96,300 | 79,357 | 2.281 s | 0.489 s |
| witness-value-column NPO FRI opening proof, 43 field columns / 86 basis columns | 81,611 | 80,645 | 60,312 | 1.057 s | 0.474 s |
...
136.6 KiB proof is too large to combine with the rest of the terminal backend.
The terminal-IO projection reaches 64.5 KiB and shows the boundary-opening
...
FRI proof at 96,994 bytes / 94.7 KiB, with about 3.18 s debug-profile prove
time and 48.9 ms verify time in the focused NPO-only fixture. It rejects both
off-window table-row IO and stale permutation-row IO while staying under the
...
measurement-driven rejection for naive composition. The next production route
must either share the trace opening with the boundary/value bridge, commit only
a relation-specific projection, or fold the lookup AIR algebra into a larger
batched proximity proof; simply appending this proof would overshoot the 100 KiB
certificate target. The LogUp/global-bus byte lookup relation remains a
...
| Production certificate is about 100 KiB | Real Tip5-L0 verifier measurement: `85,785` bytes / `83.8 KiB`, debug-profile `prove=4.952s`, `verify=3.214s`. | satisfied on the measured production fixture |
...
| Supported NPO/table rows are polynomialized into a final proximity backend | Fixed NPO table columns, verifier-side row residual evaluation, native 5-round-Tip5 FRI opening checkpoints for basis-expanded NPO columns, the optimized Tip5 lookup main trace, a 26-column terminal-IO lookup projection, terminal-IO zero-support and bridge quotients, and a combined support+bridge quotient tying lookup IO to NPO-derived IO while rejecting off-window table-row IO in one 94.7 KiB proof, a random linear-combination MLE checkpoint, and a residual-column zero-check checkpoint now exist, including Tip5/MMCS/recompose tamper tests, FRI tampered-claim rejection, stale-profile/column-set rejection, table-row IO hiding rejection, stale lookup-IO bridge rejection, and Merkle-consistent wrong-combination rejection. Current production still uses exhaustive Merkle openings rather than a complete low-degree/proximity proof over the NPO row constraints. | not complete |
...
- The terminal production checkpoint is now 85,785 bytes, or 83.8 KiB, with 60
  ...
  packing verifier-known boolean MMCS direction-bit openings, and sparse
  bitset-encoding zero extension-basis coefficients in known-index witness
  multiproofs before reconstructing full Merkle leaves. The exhaustive proof no
