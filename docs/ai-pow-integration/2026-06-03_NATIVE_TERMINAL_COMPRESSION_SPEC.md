...
| component | bytes |
|---|---:|
| primitive R1CS row-product proof | 23,742 |
...
| compact production proof body | 86,834 |
| compact production certificate | 87,059 |

The latest debug-profile measurement is `prove=4.471 s, verify=3.454 s` for
...
measured run produced an `87,059` byte / `85.0 KiB` serialized certificate,
...
the empty shell internally before checking the round multiproofs. This puts the
production certificate below the 100 KiB gate while retaining the required 60
pure-query bits. The remaining
...
| component | bytes |
|---|---:|
| sparse R1CS matrix sumcheck proof | 22,490 |
| R1CS row-product sumcheck proof | 23,742 |
| row-product rounds | 846 |
| matrix-sumcheck rounds | 724 |
| assignment evaluation proof | 22,113 |
| assignment public-prefix proof | 477 |
| assignment fold commitment roots | 579 |
| assignment fold query indices | 0 |
| assignment fold round multiproofs | 21,036 |

The latest debug-profile measurements are `prove=2.322 s, verify=1.488 s` for
the matrix-vector component and `prove=2.304 s, verify=1.482 s` for the
...
still incomplete by itself: `60,581 + 23,742` bytes leaves about 16 KiB
...
| Production certificate is about 100 KiB | Latest real Tip5-L0 verifier measurement: `87,059` bytes / `85.0 KiB`, debug-profile `prove=4.471s`, `verify=3.454s`. | satisfied on the measured production fixture |
...
- The terminal production checkpoint is now measured at 87,059 bytes, or 85.0 KiB, with 60
