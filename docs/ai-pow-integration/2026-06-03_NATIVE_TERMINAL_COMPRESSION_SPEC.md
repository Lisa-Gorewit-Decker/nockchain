...
- `TerminalNpoTip5LookupLogupQuotientProof`: a committed low-degree split
  ...
  field limbs. The focused native/Rayon integrated backend benchmark measured
  `94,016` bytes / `91.8 KiB`, compact FRI payload `76,604` bytes /
  `74.8 KiB`, `prove=23.161s`, and `verify=48.6ms`. The decompressor rejects
  non-canonical packed digest limbs before restoring the ordinary Plonky3 FRI
  proof, so this change is representation-only for the verifier algebra. An
...
| Production certificate is about 100 KiB | Earlier sub-100 KiB fixtures did not cover the complete sound terminal polynomial/proximity backend. After the row-per-round Tip5 lookup trace rewrite, production NPO-IO LogUp grouping / FRI arity pass, and fixed-width canonical Merkle path digest packing, the current sound integrated Tip5 lookup/NPO-IO LogUp direct trace-NPO-IO checkpoint at the canonical 60 pure-query bits measures `94,016` bytes / `91.8 KiB`, compact FRI payload `76,604` bytes / `74.8 KiB`, `prove=23.161s`, `verify=48.6ms`. This clears the ~100 KiB and ~30 s checkpoint targets for the current integrated polynomial/proximity measurement. | satisfied for the current integrated checkpoint |
...
- Completion status: the exhaustive-Merkle certificate meets the size and
  ...
  checkpoint now measures `94,016` bytes / `91.8 KiB` with `prove=23.161s`.
  The broader goal remains open until the terminal backend either implements
  the final polynomial/proximity argument for supported NPO/table rows with a
  complete written soundness theorem or the project explicitly accepts the
  current exhaustive-Merkle NPO verifier as the production terminal backend
  with that theorem.
