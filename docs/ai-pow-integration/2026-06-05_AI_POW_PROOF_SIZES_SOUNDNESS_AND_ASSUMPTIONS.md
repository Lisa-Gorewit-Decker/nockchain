...
- compact batch-STARK L2 over fast statement-bound L1: the current committed
  ...
  **126,862 bytes** compact wrapper / **125,979 bytes** body but raises cached
  ...
  as **141,103** canonical postcard bytes. The remaining gaps are Hoon verifier
  wiring, production-pinned expected digest selection, and the measured time
  reduction.
...
2. The compact final-layer batch-STARK L2 path is the committed primary
   ...
   `%ai-pow` artifact measured at `141,916` jammed bytes. It is not yet wired
   through the Hoon verifier path and does not yet have a production-pinned
   expected verifier-key/setup digest.
