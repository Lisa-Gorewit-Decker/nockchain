...
| Packed one-row-per-permutation Tip5 lookup trace profile | Implemented diagnostic/data-source checkpoint for the next fused native-terminal NPO backend; not an accepted production proof yet | The packed profile reuses the existing lookup-trace generator and packs each Tip5 permutation's five round rows into one row with width `500`. On the actual PROD composite verifier relation, the packed shape is `8,081` useful Tip5 rows padded to `8,192`, `1,310,720` LogUp byte tuples after zero-input padding, and `65,536` algebra quotient rows. The current row-per-round lookup trace has width `117`, `65,536` rows, and `524,288` quotient rows. This is the first implemented route that removes the full-composite `8x` Tip5 AIR quotient-domain blowup while preserving the lookup-table encoding source. Packed AIR, packed byte LogUp, packed NPO-IO projection, and selected-to-packed bridge checkpoints now verify standalone; the remaining production work is the final fused opening shape. | `RUSTFLAGS="-C target-cpu=native" cargo test -p ai-pow-zk --release --features recursion terminal_npo_polynomial_layout_for_prod_baseline_composite_is_available -- --ignored --nocapture`, 2026-06-06 |
| Packed one-row-per-permutation Tip5 AIR algebra quotient checkpoint | Diagnostic only; proves the packed trace's internal Tip5 permutation algebra, but not the byte-table LogUp relation or bridge endpoints in the same theorem | On the actual PROD composite verifier relation, the standalone packed algebra proof verifies at `136,810` bytes / `133.6 KiB`: compact FRI `126,599` bytes, opened full-trace zeta values `10,004` bytes, opened quotient `40` bytes. It uses the packed `8,192 x 500` trace and `65,536` quotient rows. Prove `22.718s`, verify `11.062s`, total diagnostic wall `76.356s` including L0 prove `9.543s`, terminal compile `11.078s`, packed trace build `3.896s`, packed trace root `5.270s`, and prelude `12.515s`. This confirms the quotient-domain reduction finishes where the row-per-round integrated proof stayed inside `air_quotient_matrix`, but the standalone full-trace opening is too large to append to the `151,448` byte merged value-bridge checkpoint. The next production route must fuse packed AIR, packed byte LogUp, packed projection, and bridge openings under one transcript rather than serialize this proof as an extra component. | `RUSTFLAGS="-C target-cpu=native" cargo test -p ai-pow-zk --release --features recursion terminal_packed_tip5_air_algebra_candidate_for_prod_baseline_measures -- --ignored --nocapture`, 2026-06-06 |
| Packed one-row-per-permutation Tip5 byte-table LogUp checkpoint | Diagnostic only; proves every packed split `(b,c)` byte pair is in the verifier-fixed Tip5 lookup table, but does not prove internal AIR algebra or bridge endpoints in the same theorem | On the actual PROD composite verifier relation, the standalone packed LogUp proof verifies at `167,018` bytes / `163.1 KiB`: compact FRI `154,339` bytes, opened full-trace zeta values `9,964` bytes, table opening `22` bytes, accumulator openings `1,849` bytes, opened quotient `42` bytes. It uses the packed `8,192 x 500` trace, one committed table-multiplicity column, 46 accumulator basis columns, `1,310,720` LogUp byte tuples, and `65,536` quotient rows. Prove `22.442s`, verify `10.821s`, total diagnostic wall `72.545s` including L0 prove `8.358s`, terminal compile `10.912s`, packed trace build `3.824s`, packed trace root `4.934s`, and prelude `10.981s`. This closes the standalone packed byte-table soundness gap, but the standalone proof is already above the relaxed `~150 KiB` target and cannot be appended to the merged value-bridge checkpoint. The next production route must fuse packed AIR, packed LogUp, packed projection, and bridge openings under one transcript. | `RUSTFLAGS="-C target-cpu=native" cargo test -p ai-pow-zk --release --features recursion terminal_packed_tip5_logup_candidate_for_prod_baseline_measures -- --ignored --nocapture`, 2026-06-06 |
| Packed one-row-per-permutation Tip5 NPO-IO projection checkpoint | Diagnostic only; proves the 26-column packed NPO-IO projection is derived from the committed packed trace, but does not by itself bind those lanes to the selected NPO values | On the actual PROD composite verifier relation, the standalone projection proof verifies at `149,525` bytes / `146.0 KiB`: compact FRI `138,727` bytes, opened full-trace zeta values `10,007` bytes, NPO-IO opening `527` bytes, opened quotient `7` bytes. It uses the packed `8,192 x 500` trace, 26 NPO-IO columns, and `16,384` projection quotient rows. Prove `20.379s`, verify `10.692s`, total diagnostic wall `70.198s` including L0 prove, terminal compile `10.827s`, packed trace build `3.823s`, packed projection root `5.256s`, and prelude `10.748s`. This closes the packed trace-to-projection binding, but it is still standalone evidence rather than an appendable production component. | `RUSTFLAGS="-C target-cpu=native" cargo test -p ai-pow-zk --release --features recursion terminal_packed_tip5_npo_io_projection_candidate_for_prod_baseline_measures -- --ignored --nocapture`, 2026-06-06 |
...
| Packed selected-to-packed-trace NPO-IO bridge checkpoint | Diagnostic only; proves lane-selector-aware multiset equality between selected NPO-value IO lanes and the packed trace's semantic NPO-IO lanes without committing to a separate packed NPO-IO projection. The verifier derives the 16 input lanes and 10 final-output lanes from the opened packed trace at `zeta`. | On the actual PROD composite verifier relation, the direct bridge proof verifies at `213,546` bytes / `208.5 KiB`: compact FRI `199,459` bytes, selected lookup opening `1,705` bytes, packed trace opening `10,021` bytes, selected/packed accumulator openings `722 + 717` bytes, selected/packed quotient openings `42 + 43` bytes. It uses `14,049` selected rows padded to `16,384`, `8,081` packed rows padded to `8,192`, packed trace width `500`, selected quotient rows `65,536`, and packed quotient rows `32,768`. Prove `31.997s`, verify `14.054s`, total diagnostic wall `92.628s` including L0 prove `7.848s`, terminal compile `10.613s`, NPO columns `4.519s`, packed trace `3.687s`, endpoint roots `9.214s`, and prelude `10.386s`. This removes the projection commitment/domain and is smaller than naive projection+selected fusion, but is still too large standalone because it opens the full packed trace. It is useful only if fused with packed AIR/LogUp so that full packed-trace opening is shared. | `RUSTFLAGS="-C target-cpu=native" cargo test -p ai-pow-zk --release --features recursion terminal_packed_tip5_selected_npo_trace_bridge_candidate_for_prod_baseline_measures -- --ignored --nocapture`, 2026-06-06 |
| Packed AIR+LogUp selected-to-packed-trace shared theorem checkpoint | Negative diagnostic; proves packed Tip5 AIR algebra, packed byte-table LogUp, and lane-selector selected NPO-value to packed trace-lane binding in one transcript with one shared packed trace opening. It is sound for those included relations, and same-phase PCS input-batch coalescing is a real compression improvement, but this is still not a viable production proof shape. | On the actual PROD composite verifier relation, the coalesced shared theorem verifies at `208,799` bytes / `203.9 KiB`: compact FRI `192,253` bytes, selected lookup opening `1,693` bytes, packed trace opening `9,992` bytes, AIR quotient opening `41` bytes, LogUp table opening `20` bytes, LogUp accumulator opening `1,849` bytes, LogUp quotient opening `42` bytes, selected/packed bridge accumulator openings `727 + 729` bytes, and selected/packed bridge quotient openings `42 + 40` bytes. It uses AIR quotient rows `65,536`, LogUp quotient rows `65,536`, selected bridge quotient rows `65,536`, and packed bridge quotient rows `32,768`. Prove `33.313s`, verify `14.073s`, total diagnostic wall `94.365s`. The immediately preceding uncoalesced version measured `273,113` bytes, compact FRI `256,261` bytes, and prove `36.590s`, so coalescing removed about `64.3 KiB` but still leaves too much full-trace/accumulator payload. | `RUSTFLAGS="-C target-cpu=native" cargo test -p ai-pow-zk --release --features recursion terminal_packed_tip5_air_logup_selected_trace_bridge_candidate_for_prod_baseline_measures -- --ignored --nocapture`, 2026-06-06 |
...
- full `ai-pow-zk` composite-verifier integrated-LogUp polynomial NPO
  ...
  **7.600s** prelude construction, and **14.435s** primitive proving before the
  ...
  domain from **524,288** to **65,536** rows. The standalone packed algebra
  proof now verifies at **136,810 bytes** with **22.718s** proving, so it is a
  real time improvement over the row-per-round integrated proof. The standalone
  packed byte-table LogUp proof verifies at **167,018 bytes** with **22.442s**
  proving, proving the missing table membership binding but exceeding the
  relaxed size target by itself. The standalone selected-to-packed NPO-IO
  ...
  removes the projection commitment/domain and verifies at **213,546 bytes**
  with **31.997s** proving, smaller than the naive projection+selected fused
  proof but still too large as a standalone artifact because it opens the full
  500-column packed trace. The first shared packed-trace AIR+LogUp
  selected-trace theorem also verifies. Same-phase PCS input-batch coalescing
  improves it from **273,113 bytes** / **36.590s** proving to **208,799
  bytes** / **33.313s** proving, with compact FRI reduced from **256,261** to
  **192,253 bytes**. These are sound checkpoints, not appendable production
  proof bodies; the production route now has to reduce the remaining
  full-trace/accumulator opening payload and fold a leaner packed Tip5 binding
  into the merged value-bridge theorem.
  Additional prelude commitment caching and low-level quotient-loop cleanup
  alone will not make this candidate meet **30s total**.
