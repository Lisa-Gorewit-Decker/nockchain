...
production wire budget and must not be treated as the production block/wire
artifact.
...
| Selected compact L2 table-packing sweep | Verifies low/default/high ALU packing variants through the same verifier-owned compact context; identifies a size reserve but rules out simple ALU lane retuning as the total-time lever | Same fast L1 in one run: cached L1 prove `15.668s`. L2 `alu_lanes=2,horner_k=5`: compact wrapper `126,862` bytes, body `125,979`, cached L2 prove `30.801s`, ALU shape `38` main cols x `23,136` rows. `alu_lanes=4`: `133,736` bytes, body `132,851`, cached L2 prove `30.875s`, ALU `54 x 11,568`. Baseline `alu_lanes=8`: `143,762` bytes, body `142,878`, cached L2 prove `28.530s`, ALU `86 x 5,960`. `alu_lanes=16`: `163,875` bytes, cached L2 prove `30.945s`; `alu_lanes=32`: `203,804` bytes, cached L2 prove `36.279s`; `alu_lanes=16,horner_k=8`: `167,211` bytes, cached L2 prove `32.108s` | `RUSTFLAGS="-C target-cpu=native" cargo test -p ai-pow-zk --release --features recursion selected_fast_l1_compact_l2_table_packing_sweep_for_test_pearl -- --ignored --nocapture`, 2026-06-07 |
...
| Promoted compact batch recursive certificate API | Current committed production candidate inside `ai-pow-zk`; typed certificate carries a Tip5 verifier-key/setup digest plus the final L2 compact body, while metadata/setup, FRI shape, and public-value derivation stay verifier-owned. Now has Rust bridge builders, cached compact builder variants, miner noun packaging, and a Rust compact artifact verifier that requires verifier-owned context plus a caller-supplied expected digest. That expected digest has an explicit canonical production-config encoding: five Goldilocks limbs as 40 little-endian bytes. Hoon verifier wiring and the production source for the pinned digest remain open. | Encoded `AiPowCompactBatchRecursiveCertificate` `142,274` bytes. Timings: L1 circuit build `126ms`, L1 outer prove `19.223s`, L2 prep `9.440s`, L2 prove `29.743s`, compact body construction `3ms`, compact verify `37ms`, uncached proof wall `58.603s`. Decoded certificate verifies; wrong public inputs, wrong certificate digest, and stale context digest reject. The compact prover cache now owns reusable L2 verifier/AIR setup, shares setup with the returned verifier context, and rejects stale L1 metadata/setup before reuse. The miner noun integration preserves those canonical postcard bytes as a bounded proof-node atom instead of expanding the compact proof into a serde tree. The full Pearl-compatible `%ai-pow` artifact now measures `141,916` jammed bytes (`138.59 KiB`) with a `141,103` byte compact certificate (`137.80 KiB`); bounded decode, statement precheck, canonical compact byte-node re-encoding, decoded compact verification, jammed compact verification, wrong expected-digest rejection, and wrong-length/noncanonical expected-digest byte rejection pass. | `RUSTFLAGS="-C target-cpu=native" cargo test -p ai-pow-zk --release --features recursion compact_batch_recursive_certificate_round_trip_for_test_pearl -- --ignored --nocapture`; `RUSTFLAGS="-C target-cpu=native" cargo test -p ai-pow-miner --release --features node real_compact_pearl_merge_artifact_jam_size_for_selected_route -- --ignored --nocapture`; `RUSTFLAGS="-C target-cpu=native" cargo check -p ai-pow --features zk`; `RUSTFLAGS="-C target-cpu=native" cargo check -p ai-pow-miner --features node`, 2026-06-07 |
...
| Compact packed one-row-per-permutation Tip5 lookup trace profile | Implemented diagnostic/data-source checkpoint for the next fused native-terminal NPO backend; not an accepted production proof yet | The packed profile reuses the existing lookup-trace generator and packs each Tip5 permutation's five round rows into one row. It stores explicit round inputs only for round 0; rounds 1-4 read their inputs from the previous round's output columns. On the actual PROD composite verifier relation, the packed shape is now `8,081` useful Tip5 rows padded to `8,192`, width `436`, `1,310,720` LogUp byte tuples after zero-input padding, and `65,536` algebra quotient rows. The current row-per-round lookup trace has width `117`, `65,536` rows, and `524,288` quotient rows. This removes the full-composite `8x` Tip5 AIR quotient-domain blowup while preserving the lookup-table encoding source and trimming duplicate input columns. | `RUSTFLAGS="-C target-cpu=native" cargo test -p ai-pow-zk --release --features recursion terminal_packed_tip5_air_logup_selected_trace_bridge_candidate_for_prod_baseline_measures -- --ignored --nocapture`, 2026-06-06 |
| Compact packed one-row-per-permutation Tip5 AIR algebra quotient checkpoint | Diagnostic only; proves the packed trace's internal Tip5 permutation algebra, but not the byte-table LogUp relation or bridge endpoints in the same theorem | On the actual PROD composite verifier relation, the standalone packed algebra proof verifies at `129,471` bytes / `126.4 KiB`: compact FRI `120,507` bytes, opened full-trace zeta values `8,756` bytes, opened quotient `41` bytes. It uses the compact packed `8,192 x 436` trace and `65,536` quotient rows. Prove `19.875s`, verify `10.402s`, total diagnostic wall `67.239s` including L0 prove `7.979s`, terminal compile `10.321s`, packed trace build `3.745s`, packed trace root `4.195s`, and prelude `10.418s`. This confirms the quotient-domain and duplicate-input reductions help, but the standalone full-trace opening is still too large to append to the `151,448` byte merged value-bridge checkpoint. | `RUSTFLAGS="-C target-cpu=native" cargo test -p ai-pow-zk --release --features recursion terminal_packed_tip5_air_algebra_candidate_for_prod_baseline_measures -- --ignored --nocapture`, 2026-06-06 |
| Compact packed one-row-per-permutation Tip5 byte-table LogUp checkpoint | Diagnostic only; proves every packed split `(b,c)` byte pair is in the verifier-fixed Tip5 lookup table, but does not prove internal AIR algebra or bridge endpoints in the same theorem | On the actual PROD composite verifier relation, the standalone packed LogUp proof verifies at `155,974` bytes / `152.3 KiB`: compact FRI `144,547` bytes, opened full-trace zeta values `8,709` bytes, table opening `21` bytes, accumulator openings `1,848` bytes, opened quotient `42` bytes. It uses the compact packed `8,192 x 436` trace, one committed table-multiplicity column, 46 accumulator basis columns, `1,310,720` LogUp byte tuples, and `65,536` quotient rows. Prove `22.284s`, verify `11.055s`, total diagnostic wall `72.871s` including L0 prove `8.934s`, terminal compile `10.925s`, packed trace build `3.941s`, packed trace root `4.447s`, and prelude `10.962s`. This closes the standalone packed byte-table soundness gap, but the standalone proof is still above the relaxed `~150 KiB` target and cannot be appended to the merged value-bridge checkpoint. | `RUSTFLAGS="-C target-cpu=native" cargo test -p ai-pow-zk --release --features recursion terminal_packed_tip5_logup_candidate_for_prod_baseline_measures -- --ignored --nocapture`, 2026-06-06 |
| Packed one-row-per-permutation Tip5 NPO-IO projection checkpoint | Diagnostic only; proves the 26-column packed NPO-IO projection is derived from the committed packed trace, but does not by itself bind those lanes to the selected NPO values | Pre-compaction width-500 measurement: standalone projection proof verifies at `149,525` bytes / `146.0 KiB`: compact FRI `138,727` bytes, opened full-trace zeta values `10,007` bytes, NPO-IO opening `527` bytes, opened quotient `7` bytes. It used the packed `8,192 x 500` trace, 26 NPO-IO columns, and `16,384` projection quotient rows. Prove `20.379s`, verify `10.692s`, total diagnostic wall `70.198s`. This row has not yet been rerun after compacting the packed trace to width `436`; the sharper direct selected-to-packed-trace diagnostic has been rerun below. | `RUSTFLAGS="-C target-cpu=native" cargo test -p ai-pow-zk --release --features recursion terminal_packed_tip5_npo_io_projection_candidate_for_prod_baseline_measures -- --ignored --nocapture`, 2026-06-06 |
| Packed one-row-per-permutation Tip5 selected-to-packed NPO-IO bridge checkpoint | Diagnostic only; proves lane-selector-aware multiset equality between selected NPO-value IO lanes and the packed NPO-IO projection endpoint, but only as a standalone bridge proof | On the actual PROD composite verifier relation, the standalone bridge proof verifies at `137,355` bytes / `134.1 KiB`: compact FRI `132,756` bytes, selected lookup opening `1,691` bytes, packed NPO-IO opening `525` bytes, selected/packed accumulator openings `721 + 729` bytes, selected/packed quotient openings `39 + 40` bytes. It uses `14,049` selected rows padded to `16,384`, `8,081` packed rows padded to `8,192`, 26 packed NPO-IO columns, selected quotient rows `65,536`, packed quotient rows `32,768`, and a 3-lane LogUp grouping. Prove `28.526s`, verify `14.510s`, total diagnostic wall `86.643s` including L0 prove `8.188s`, terminal compile `10.751s`, NPO columns `4.623s`, packed trace `3.755s`, bridge endpoint roots `5.175s`, and prelude `10.798s`. This confirms the output-present selector bridge is sound and now meets the relaxed standalone size/time numbers; the final route must fuse its openings with the merged value bridge and packed trace/projection openings. | `RUSTFLAGS="-C target-cpu=native" cargo test -p ai-pow-zk --release --features recursion terminal_packed_tip5_selected_npo_io_bridge_candidate_for_prod_baseline_measures -- --ignored --nocapture`, 2026-06-06 |
| Packed NPO-IO projection plus selected bridge fused checkpoint | Negative diagnostic; proves the packed trace-to-NPO-IO projection and selected-to-packed bridge in one transcript/FRI opening, but demonstrates that naive multi-domain batching is not the final production fusion shape | On the actual PROD composite verifier relation, the fused proof verifies at `243,516` bytes / `237.8 KiB`: compact FRI `228,782` bytes, selected lookup opening `1,702` bytes, packed trace opening `9,955` bytes, packed NPO-IO opening `521` bytes, projection quotient opening `7` bytes, selected/packed accumulator openings `725 + 721` bytes, selected/packed quotient openings `40 + 41` bytes. It uses projection quotient rows `16,384`, selected bridge quotient rows `65,536`, and packed bridge quotient rows `32,768`. Prove `35.423s`, verify `14.835s`, total diagnostic wall `100.919s`. This closes both relations in one proof, but the compact-FRI payload grows too much; final fusion needs algebraic composition/domain reduction, not just one larger PCS opening batch. | `RUSTFLAGS="-C target-cpu=native" cargo test -p ai-pow-zk --release --features recursion terminal_packed_tip5_projection_selected_bridge_fused_candidate_for_prod_baseline_measures -- --ignored --nocapture`, 2026-06-06 |
| Compact packed selected-to-packed-trace NPO-IO bridge checkpoint | Diagnostic only; proves lane-selector-aware multiset equality between selected NPO-value IO lanes and the packed trace's semantic NPO-IO lanes without committing to a separate packed NPO-IO projection. The verifier derives the 16 input lanes and 10 final-output lanes from the opened packed trace at `zeta`. | On the actual PROD composite verifier relation, the direct bridge proof verifies at `205,950` bytes / `201.1 KiB`: compact FRI `193,160` bytes, selected lookup opening `1,707` bytes, packed trace opening `8,718` bytes, selected/packed accumulator openings `720 + 725` bytes, selected/packed quotient openings `42 + 40` bytes. It uses `14,049` selected rows padded to `16,384`, `8,081` packed rows padded to `8,192`, compact packed trace width `436`, selected quotient rows `65,536`, and packed quotient rows `32,768`. Prove `35.863s`, verify `14.955s`, total diagnostic wall `102.241s`. This removes the projection commitment/domain and is smaller than naive projection+selected fusion, but is still too large standalone because it opens the full compact packed trace. | `RUSTFLAGS="-C target-cpu=native" cargo test -p ai-pow-zk --release --features recursion terminal_packed_tip5_selected_npo_trace_bridge_candidate_for_prod_baseline_measures -- --ignored --nocapture`, 2026-06-06 |
| Compact packed AIR+LogUp selected-to-packed-trace shared theorem checkpoint | Negative diagnostic; proves packed Tip5 AIR algebra, packed byte-table LogUp, and lane-selector selected NPO-value to packed trace-lane binding in one transcript with one shared compact packed trace opening. It is sound for those included relations, and same-phase PCS input-batch coalescing plus duplicate-input removal are real compression improvements, but this is still not a viable production proof shape. | On the actual PROD composite verifier relation, the compact-trace coalesced shared theorem verifies at `198,287` bytes / `193.6 KiB`: compact FRI `183,018` bytes, selected lookup opening `1,698` bytes, packed trace opening `8,728` bytes, AIR quotient opening `41` bytes, LogUp table opening `21` bytes, LogUp accumulator opening `1,840` bytes, LogUp quotient opening `42` bytes, selected/packed bridge accumulator openings `716 + 724` bytes, and selected/packed bridge quotient openings `42 + 41` bytes. It uses AIR quotient rows `65,536`, LogUp quotient rows `65,536`, selected bridge quotient rows `65,536`, and packed bridge quotient rows `32,768`. Prove `33.277s`, verify `14.358s`, total diagnostic wall `94.874s`. The uncoalesced width-500 version measured `273,113` bytes and the first coalesced width-500 version measured `208,799` bytes, so the latest compaction removes another `10.5 KiB` but still leaves too much full-trace/accumulator payload. | `RUSTFLAGS="-C target-cpu=native" cargo test -p ai-pow-zk --release --features recursion terminal_packed_tip5_air_logup_selected_trace_bridge_candidate_for_prod_baseline_measures -- --ignored --nocapture`, 2026-06-06 |
| Merged value bridge plus packed support fusion floor | Negative diagnostic; builds the merged value bridge and compact packed AIR+LogUp selected-trace support theorem over one terminal circuit and one shared prelude, then computes an optimistic lower bound for simple single-FRI fusion, a paired 16-bit Tip5 lookup payload estimate, and a zero-support-FRI structural floor. This is not a production proof shape; it quantifies whether ordinary transcript/FRI sharing, byte-pair lookup packing, or support-FRI elimination can plausibly meet the relaxed target without reducing the base proof. | On the actual PROD composite verifier relation, the current appended body is `343,209` bytes. The deliberately optimistic single-FRI floor is `252,753` bytes after keeping only the larger compact FRI body and subtracting duplicate selected-lookup profile, commitment, and opening payload. That is `99,153` bytes over a binary `150 KiB` gate. Component split: prelude `286`, primitive R1CS `54,927`, merged value bridge `90,819` with compact FRI `88,614`, packed support `197,177` with compact FRI `181,917`, merged non-FRI `2,205`, packed-support non-FRI `15,260`, optimistic selected-binding dedup `1,842`. The packed-support compact FRI split is input batches `127,180` bytes, input opened values `86,391`, input Merkle `40,788`, commit rounds `54,313`, commit-round Merkle `45,047`, query count `15`. Input batches: selected lookup `21,459`, packed trace/table `72,133`, accumulators `21,934`, quotients `11,653`. The paired 16-bit lookup estimate reduces trace width `436 -> 276`, LogUp accumulator basis `46 -> 24`, estimates FRI opened-value savings `25,912` bytes and non-FRI zeta-opening savings `4,082` bytes, but still leaves an optimistic floor of `222,759` bytes before two-domain table Merkle overhead or new quotient-shape cost. The tighter structural floor is `146,032` bytes for prelude + primitive R1CS + merged value bridge; this leaves only `7,568` bytes under binary `150 KiB`. Current support metadata after duplicate selected-binding dedup is `13,418` bytes, so a zero-support-FRI floor is `159,450` bytes. Paired-lookup metadata lowers that to `9,336` bytes, but the zero-support-FRI floor is still `155,368` bytes. Prove subproofs in the latest run: primitive `9.171s`, merged value bridge `2.373s`, packed support `33.912s`, total subproof prove `45.457s`; total diagnostic wall `171.980s`. Ordinary single-FRI fusion, paired lookup alone, and support-FRI elimination without metadata/base reduction are therefore ruled out; the next route must either reduce the primitive/merged base or make Tip5 support almost metadata-free inside a genuinely merged theorem. | `RUSTFLAGS="-C target-cpu=native" cargo test -p ai-pow-zk --release --features recursion terminal_merged_value_bridge_packed_support_fusion_floor_for_prod_baseline_measures -- --ignored --nocapture`, 2026-06-06 |
...
- compact batch-STARK L2 over fast statement-bound L1: the current committed
  production candidate. The latest best measured complete row uses L1
  `alu_lanes=4,horner_k=5` and measures **143,106 bytes** for the actual
  compact wrapper and **142,225 bytes** for the metadata-free compact body, so
  the relaxed size gate is plausibly in range. The same run still measures
  **43.584s** cached serial L1+L2 per-attempt proving with both prep stages
  cached, so the route is not yet production-complete. The baseline selected
  row remains close at **143,762 bytes** / **142,878 bytes** body and about
  **44.031s** cached serial in the focused rerun. The latest L2 cached proving
  alone is under the relaxed time gate if counted without L1. Verified
  table-packing sweeps find smaller size reserves: L2 `alu_lanes=2` gives
  **126,862 bytes** compact wrapper / **125,979 bytes** body but raises cached
  L2 proving to **30.801s**, and L1 `alu_lanes=2` gives **141,148 bytes**
  compact wrapper / **140,260 bytes** body with **44.391s** cached serial.
  Lower ALU lanes are therefore size knobs, not the time route. The prior
  ...
  recursive-verifier witness execution or FRI query work. This route is now
  promoted inside `ai-pow-zk` as `AiPowCompactBatchRecursiveCertificate`; the
  encoded crate-level certificate measures **142,274 bytes** including its
  Tip5 verifier-key/setup digest, decoded verification passes, wrong public
  inputs reject, wrong certificate digest rejects, stale context digest rejects,
  and the uncached round-trip wall is **58.603s**. The miner noun boundary now
  measures **141,916 jammed bytes / 138.59 KiB** for the full
  Pearl-compatible `%ai-pow` artifact, with the compact certificate preserved
  as **141,103** canonical postcard bytes. The Rust compact artifact verifier
  now verifies decoded and jammed compact artifacts against verifier-owned
  context plus a caller-supplied expected digest and rejects a wrong expected
  digest. The expected digest is now encoded at config boundaries as 40
  canonical Goldilocks bytes, with wrong-length and noncanonical encodings
  rejected before proof verification. The compact L2 prep/cache is now exposed
  through Rust prover APIs and rejects stale L1 metadata/setup before reuse. The
  remaining gaps are Hoon verifier wiring, production pinned-digest source,
  production cache warm-up/lifecycle, and the measured time reduction.
...
- native terminal recursive fixture: **85,948 bytes / 83.9 KiB**, satisfying
  ...
  Tip5 verifier fixture, but now fallback evidence rather than the primary
  production route;
...
- full `ai-pow-zk` composite-verifier integrated-LogUp polynomial NPO
  ...
  trace-bundle PCS data reuse, a batched-LDE AIR quotient builder, and a
  prover-only folded-AIR evaluator cleanup that parallelizes quotient-row
  evaluation and removes per-row heap allocations. The recursion-crate
  synthetic backend remains promising: the production-candidate body is
  unchanged at **96,017 bytes / 93.8 KiB** while primitive+NPO prove time is now
  **9.918s** in the focused run. On the full composite, selected+lookup reuse
  removes another duplicated-work blocker: merged value-bridge proving falls to
  **2.340s** after the selected+lookup root has already been prepared. This is
  still not enough to claim production success. The latest full-composite
  diagnostic spent **29.834s** in L0 proving before terminal work, plus
  **7.543s** terminal compile, **5.793s** assignment commitment, **11.075s**
  selected+lookup root construction, **6.261s** trace-bundle root construction,
  **7.600s** prelude construction, and **14.435s** primitive proving before the
  integrated Tip5 LogUp subproof remained inside
  `terminal_npo_integrated_logup.air_quotient_matrix` for more than 90 seconds
  and was stopped. The next sound optimization must reduce or specialize the
  full Tip5 AIR quotient work; the new packed one-row-per-permutation lookup
  trace, algebra quotient, and byte-table LogUp quotient are the concrete
  specialization checkpoints, reducing the actual PROD Tip5 algebra quotient
  domain from **524,288** to **65,536** rows. The compact packed trace now
  omits duplicate explicit round-input columns for rounds 1-4, reducing packed
  width from **500** to **436**. The standalone packed algebra proof now
  verifies at **129,471 bytes** with **19.875s** proving, so it is a real time
  improvement over the row-per-round integrated proof. The standalone packed
  byte-table LogUp proof verifies at **155,974 bytes** with **22.284s**
  proving, proving the missing table membership binding but still exceeding
  the relaxed size target by itself. The standalone selected-to-packed NPO-IO
  bridge now verifies at **137,355 bytes** with **28.526s** proving after the
  3-lane LogUp grouping change. The direct selected-to-packed-trace bridge
  removes the projection commitment/domain and verifies at **205,950 bytes**
  with **35.863s** proving, smaller than the naive projection+selected fused
  proof but still too large as a standalone artifact because it opens the full
  436-column compact packed trace. The first shared packed-trace AIR+LogUp
  selected-trace theorem also verifies. Same-phase PCS input-batch coalescing
  and compact duplicate-input removal improve it from **273,113 bytes** /
  **36.590s** proving to **198,287 bytes** / **33.277s** proving, with compact
  FRI reduced from **256,261** to **183,018 bytes**. These are sound
  checkpoints, not appendable production proof bodies. A new merged-value plus
  packed-support fusion-floor diagnostic makes the negative result explicit:
  current append is **343,209 bytes**, and even an optimistic single-FRI floor
  after duplicate selected-binding subtraction is **252,753 bytes**. The
  paired 16-bit lookup payload estimate reduces the trace width floor to
  **276** and LogUp accumulator basis columns to **24**, but it saves only
  **25,912 bytes** of FRI opened values plus **4,082 bytes** of non-FRI zeta
  openings and still leaves an optimistic floor of **222,759 bytes** before
  new table-domain overhead. The structural floor is now tighter: prelude +
  primitive R1CS + merged value bridge is already **146,032 bytes**, leaving
  only **7,568 bytes** under binary **150 KiB** for all remaining sound Tip5
  support binding if the base is unchanged. Current support metadata alone is
  **13,418 bytes** after selected-binding dedup, and paired-lookup metadata is
  still **9,336 bytes**, so even a free support FRI would not fit without more
  metadata or base reduction. The native-terminal fallback route therefore has
  to remove the packed-support theorem's FRI input-batch, Merkle, accumulator,
  and/or commit-round shape itself and avoid serializing current support
  metadata, or reduce the primitive/merged base. Additional prelude commitment
  caching, transcript sharing, paired lookup alone, and low-level quotient-loop
  cleanup will not make this candidate meet **150 KiB** or **30s total**.
...
- full `ai-pow-zk` composite-verifier merged padding/value-bridge candidate:
  the next checkpoint now verifies over the actual composite relation and
  serializes to **151,448 bytes / 147.9 KiB** for `(prelude, primitive R1CS
  row-product, merged residual-zero/recompose/padding/value-bridge)`. The
  merged NPO proof alone is **93,707 bytes** with a **91,501 byte** compact FRI
  payload. This is under a binary `150 KiB` relaxed size gate and only
  **1,448 bytes** over a strict decimal `150,000` byte gate. After prover-work
  reuse the post-prelude proof body builds in **14.914s** serial
  (**8.372s** primitive, **6.541s** merged NPO). It is still not the production
  theorem: it now binds the `mmcs_bit`-selected bus-to-trace value projection,
  mixed value padding, `mmcs_bit` zero/boolean constraints, dynamic hidden Tip5
  padding, new-start zero constraints, Merkle capacity-zero constraints, and
  recompose value semantics, but the final stack must still include the Tip5
  lookup/AIR/LogUp relation. Full diagnostic wall time remains **108.606s**
  because setup and verification are outside the post-prelude proof-body timer.
...
generic terminal relation, not a production profile recommendation. That
parameter discussion now applies only to the native-terminal fallback. The
active compact batch-STARK route uses fast L1 `lb=3,nq=20` and selected L2
`lb=5,nq=12` because it is the best measured route inside the relaxed size
envelope.
...
latest release/native run measures **4.772s** reusable L1 prep, **15.305s**
cached L1 proving, **20.077s** total L1 proving, **9.364s** reusable L2 prep,
**28.726s** cached L2 proving, and **38.090s** uncached L2 total. That
...
tuning. A later selected L2 table-packing sweep also rules out simple ALU lane
retuning as that time lever: smaller lanes create useful byte margin but raise
cached L2 prove time, while larger lanes widen the ALU table and are worse on
both size and time.
...
There are currently four recursive proof paths in the tree, and they must
not be conflated:
...
2. The compact final-layer batch-STARK L2 path is the committed primary
   production candidate and is now promoted inside `ai-pow-zk` as
   `AiPowCompactBatchRecursiveCertificate` plus
   ...
   `%ai-pow` artifact measured at `141,916` jammed bytes. The Rust compact
...
The committed compact L2 route is different from that oversized checkpoint. It
...
cached serial L1+L2 proving is still **44.031s** with both prep stages cached.
...
builder and measures `141,916` jammed bytes for the full `%ai-pow` artifact;
...
plus NPO candidate body measures `96,017` bytes with `9.918s` total prove
time after prepared selected+lookup reuse, the batched-LDE AIR quotient
builder, and folded-AIR evaluator cleanup. That is the strongest evidence for
the proof shape: one native terminal backend, one prelude, a merged
padding/value bridge, and an integrated AIR+LogUp/support/NPO-IO LogUp proof
with bundled masked IO. The full composite
...
already spent `14.435s` in primitive proving plus `2.340s` in the cached merged
padding/value bridge before the integrated Tip5 LogUp subproof remained inside
`terminal_npo_integrated_logup.air_quotient_matrix` for more than 90 seconds
after quotient-loop cleanup.
...
The follow-up merged padding/value-bridge checkpoint includes the
residual-zero layer, the recompose quotient, the mixed value-padding quotient,
and the NPO-row value bridge in one FRI-native proof, then serializes it with
the same terminal prelude and primitive row-product component. It verifies over
the full composite relation with body `151,448` bytes: `240` bytes of prelude,
`57,501` bytes of primitive proof, and `93,707` bytes of merged NPO proof with
`91,501` bytes of compact FRI material. After reusing primitive
relation/assignment data, preparing merged NPO data once, using
prelude-checked prover calls, and evaluating both padding and value-bridge
quotients by batched coset LDE under one combined quotient commitment,
post-prelude proof-body construction is `14.914s` serial: `8.372s` for the
primitive row-product proof plus `6.541s` for the merged NPO proof. Full
diagnostic wall time is still `108.606s` because terminal compile, assignment
commitment, prepared NPO-root construction, prelude construction, verification,
and other setup are outside that proof-body timer.
...
lanes consistently with the lookup/AIR trace. The padding quotient now proves
mixed value padding, `mmcs_bit` zero-when-absent and booleanity, dynamic hidden
Tip5 padding, new-start zero constraints, Merkle capacity-zero constraints, and
recompose value semantics. Hidden Tip5 lane presence for Merkle rows is not a
verifier-fixed selector: the constraint uses the committed `mmcs_bit` to select
between normal and swapped hidden selectors, so serialized sibling lanes
`5..9` are not incorrectly zeroed on new-start Merkle rows while prior-state
lanes and capacity lanes remain constrained. The remaining production theorem
gap is the internal Tip5 lookup/AIR/LogUp relation, not the value-padding
quotient.

A prior 2026-06-06 attempt to append that padding/MMCS-bit quotient inside the
same merged FRI object failed because the hidden-selector taxonomy treated
Merkle hidden-lane presence as verifier-fixed and then spent per-point work in
`padding_quotient_matrix`. The current implementation replaces that with
dynamic `mmcs_bit` selectors and a batched LDE, reducing the production
`padding_quotient_matrix` phase to about `142ms` in the measured run.
...
prelude verification then dropped post-prelude primitive proving to `9.232s` in
the value-bridge-only run and `8.372s` in the latest padding-merged run.
...
reuse and folded-AIR evaluator cleanup made the current failure mode more
concrete:
...
composite relation. The packed Tip5 lookup trace and algebra quotient
checkpoints now give the next implementation target: keep the lookup-table
encoding, prove five rounds in one row per permutation so the full-composite
quotient domain is `65,536` rather than `524,288` rows, and then fuse the
remaining packed LogUp and selected-value bridge under the production prelude.
...
The native-terminal fallback path must keep the coefficient binding unless a
replacement is proven, and even a perfect replacement would not close the
size/time gap by itself.
...
| Hardened L1 batch-STARK checkpoint | `log_blowup=4`, `num_queries=9`, `query_pow_bits=24`, `cap_height=5` | 60 bits under mixed query/PoW Johnson accounting | Yes; acceptable for the checkpoint, not for the pure-query production target |
...
| Compact batch-STARK L2 production candidate | L1 `lb=3,nq=20,pow=0,cap=4`; L2 `lb=5,nq=12,pow=0,cap=4`; verifier-owned setup/metadata context; explicit L1 statement-digest public lanes | 60 pure FRI-query bits for each promoted batch-STARK layer under the code's Johnson accounting, plus explicit public-value/setup binding regressions | No |
| Native terminal fallback backend | `log_blowup=4`, `num_queries=15`, `query_pow_bits=0`, `max_log_arity=3`, `log_final_poly_len=0` | Intended 60 pure FRI-query bits for the terminal backend, conditionally on the selected Plonky3 FRI theorem/assumption and terminal theorem; fixture proof-size gate passes, but the full `ai-pow-zk` composite-verifier path has not met the size/time gates | No |
| End-to-end production recursive certificate target | L0 proof accepted by compact batch-STARK L2 over a statement-bound L1 proof; native terminal remains fallback | At most the minimum of the L0, L1, and L2 layers: **60 bits** | No proof-system PoW counted |
...
must either forge the Layer-0 STARK statement, forge the statement-bound L1
proof, forge the compact L2 proof that verifies that L1 statement, or break one
of the transcript/commitment/setup assumptions that bind the layers. The native
terminal backend keeps its own pure-query fallback claim, but it is no longer
the primary production route.
...
Layer-0 composite STARK
  ...
  v
Committed compact batch-STARK L2 certificate candidate
  proves:
    - the Layer-0 proof was accepted by the statement-bound L1 recursive
      verifier circuit;
    - the L1 statement digest is carried into the L2 final public lanes;
    - the verifier owns the L2 setup/metadata context and expected FRI shape;
    - preprocessed openings are restored from canonical setup rather than
      trusted as prover-supplied metadata;
    - compact FRI paths and public values verify under the canonical context.
  |
  v
Nockchain block/wire artifact target: compact batch-STARK L2 certificate
```

The native terminal path follows the same Layer-0 verifier-circuit handoff, but
proves the executed verifier circuit with the terminal backend instead of the
compact batch-STARK L2 adapter. It remains fallback evidence. The large
batch-STARK checkpoint path still proves the executed verifier circuit with
`BatchStarkProver` and remains useful for regression and fallback validation,
but not for the production wire budget.
...
- No Plonky2 proof system in production. Pearl/Plonky2 code was read only as a
  design reference for safe FRI path compression. The committed compact L2
  route uses the in-tree Plonky3 batch-STARK stack; the native terminal fallback
  is implemented in the vendored Plonky3-recursion stack.
- No claim that the multi-hundred-KiB-to-MiB L1 batch-STARK checkpoint is the production
  block/wire artifact.
- No zero-knowledge claim for the recursive certificate candidates. The native
  terminal fallback's exhaustive NPO openings reveal selected
  recursive-verifier witness material, including hidden Tip5 input lanes needed
  to recompute hidden-input rows.
...
release/native run measures **15.305 s** cached L1 proving plus **28.726 s**
cached L2 proving, or **44.031 s** cached serial L1+L2 per attempt with both
prep stages cached. The materialized
...
at **85,948 bytes / 83.9 KiB** with release prove **1.492 s** and verify
**1.181 s**, but the wired full `ai-pow-zk` composite-verifier terminal path
measures **771,249 bytes / 753.2 KiB** on the postcard wire with release prove
**80.829 s** and verify **58.825 s** even under the reduced
`lb=6,nq=10,pow=0` profile.
...
2. If the Layer-0 verifier would reject, or if the statement digest/public
   values are substituted, a valid compact route requires breaking the
   statement-bound L1 proof, the compact L2 verifier relation, FRI/PCS, Tip5
   commitments, verifier-owned setup binding, or transcript binding, or
   exploiting a bug in the recursive-verifier relation.
3. The certificate must bind public values, verifier-key/setup digest,
   metadata/profile shape, commitments, preprocessed openings, and production
   parameters before challenge derivation, so there is no intended grinding
   surface over public values, profiles, roots, setup, or query indices.

The intended weakest production soundness term remains the 60-bit pure-query
floor shared by the Layer-0, L1, and L2 proof layers. The open engineering work
is to promote the compact L2 candidate into the production certificate wire/API,
pin the verifier-key/setup digest and statement metadata, add final-artifact
rejection tests, and reduce cached serial L1+L2 proving from about **44 s** to
about **30 s** by shrinking committed recursive-verifier matrix volume. The
hardened batch-STARK checkpoint and native terminal fallback are also required
to stay cryptographically sound, but neither is the current production
size/time path.
