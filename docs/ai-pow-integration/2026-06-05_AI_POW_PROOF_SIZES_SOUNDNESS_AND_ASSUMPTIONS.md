...
| Packed one-row-per-permutation Tip5 selected-to-packed NPO-IO bridge checkpoint | Diagnostic only; proves lane-selector-aware multiset equality between selected NPO-value IO lanes and the packed NPO-IO projection endpoint, but only as a standalone bridge proof | On the actual PROD composite verifier relation, the standalone bridge proof verifies at `140,843` bytes / `137.5 KiB`: compact FRI `137,218` bytes, selected lookup opening `1,720` bytes, packed NPO-IO opening `524` bytes, selected/packed accumulator openings `322 + 322` bytes, selected/packed quotient openings `42 + 39` bytes. It uses `14,049` selected rows padded to `16,384`, `8,081` packed rows padded to `8,192`, 26 packed NPO-IO columns, selected quotient rows `131,072`, and packed quotient rows `65,536`. Prove `31.157s`, verify `14.482s`, total diagnostic wall `89.074s` including L0 prove `8.130s`, terminal compile `10.787s`, NPO columns `4.654s`, packed trace `3.756s`, bridge endpoint roots `5.125s`, and prelude `10.658s`. This confirms the output-present selector bridge is sound as a checkpoint; the final route must fuse its openings with the merged value bridge and packed trace/projection openings. | `RUSTFLAGS="-C target-cpu=native" cargo test -p ai-pow-zk --release --features recursion terminal_packed_tip5_selected_npo_io_bridge_candidate_for_prod_baseline_measures -- --ignored --nocapture`, 2026-06-06 |
...
- full `ai-pow-zk` composite-verifier integrated-LogUp polynomial NPO
  ...
  **7.600s** prelude construction, and **14.435s** primitive proving before the
  ...
  trace, algebra quotient, and byte-table LogUp quotient are the concrete specialization checkpoints,
  reducing the actual PROD Tip5 algebra quotient domain from **524,288** to
  **65,536** rows. The standalone packed algebra proof now verifies at
  **136,810 bytes** with **22.718s** proving, so it is a real time improvement
  over the row-per-round integrated proof. The standalone packed byte-table
  LogUp proof now verifies at **167,018 bytes** with **22.442s** proving,
  proving the missing table membership binding but exceeding the relaxed size
  target by itself. These are sound checkpoints, not appendable production
  proof bodies; the production route still has to fuse packed AIR, packed
  LogUp, and the selected-value bridge under one transcript/opening shape.
