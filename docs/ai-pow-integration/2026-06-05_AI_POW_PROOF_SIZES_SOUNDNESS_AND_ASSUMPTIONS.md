...
- full `ai-pow-zk` composite-verifier integrated-LogUp polynomial NPO
  ...
  **7.600s** prelude construction, and **14.435s** primitive proving before the
  ...
  trace profile is the concrete specialization checkpoint, reducing the actual
  PROD Tip5 algebra quotient domain from **524,288** to **65,536** rows before
  the packed quotient theorem is wired. Additional prelude commitment caching
  and low-level quotient-loop cleanup alone will not make this candidate meet
  **30s total**.
...
composite relation. The packed Tip5 lookup trace checkpoint now gives the next
implementation target: keep the lookup-table encoding, but prove five rounds in
one row per permutation so the full-composite quotient domain is `65,536`
rather than `524,288` rows. This is still a data-source/profile checkpoint
until its packed AIR/LogUp quotient and selected-value bridge are proven under
the production prelude.
