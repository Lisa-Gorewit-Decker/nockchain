...
- compact batch-STARK L2 over fast statement-bound L1: the current committed
  ...
  **19.123s** L1 outer proving, **28.617s** L2 proving, and **57.406s**
  ...
  through Rust prover APIs, rejects stale L1 metadata/setup before reuse, and
  is lazily warmed by the production miner after the first compact certificate
  in a process. If reuse is rejected as stale, the bridge rebuilds uncached and
  returns a fresh cache for the miner to store. The remaining gaps are Hoon
  verifier wiring, production pinned-digest source, proactive/operator cache
  policy, and the measured time reduction.
