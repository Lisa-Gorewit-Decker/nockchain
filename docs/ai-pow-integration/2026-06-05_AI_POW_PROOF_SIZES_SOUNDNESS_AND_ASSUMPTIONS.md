...
| Promoted compact batch recursive certificate API | Current committed production candidate inside `ai-pow-zk`; typed certificate carries only the final L2 compact body, while metadata/setup, FRI shape, and public-value derivation stay verifier-owned. Not yet wired through the bridge, miner noun, Hoon wire path, or a pinned verifier-key/setup digest. | Encoded `AiPowCompactBatchRecursiveCertificate` `142,225` bytes. Timings: L1 circuit build `135ms`, L1 outer prove `19.190s`, L2 prep `9.347s`, L2 prove `28.968s`, compact body construction `3ms`, compact verify `41ms`, uncached proof wall `57.691s`. Decoded certificate verifies, and wrong public inputs reject. | `RUSTFLAGS="-C target-cpu=native" cargo test -p ai-pow-zk --release --features recursion compact_batch_recursive_certificate_round_trip_for_test_pearl -- --ignored --nocapture`, 2026-06-07 |
...
- compact batch-STARK L2 over fast statement-bound L1: the current committed
  ...
  **126,862 bytes** compact wrapper / **125,979 bytes** body but raises cached
  ...
  encoded crate-level certificate measures **142,225 bytes**, decoded
  verification passes, wrong public inputs reject, and the uncached round-trip
  wall is **57.691s**. The remaining gap is bridge/miner/Hoon wiring plus
  pinned verifier-key/setup digest derivation and the measured time reduction.
...
2. The compact final-layer batch-STARK L2 path is the committed primary
   ...
   final L2 public lanes. It is not yet wired through the bridge, miner noun,
   Hoon wire path, or a pinned verifier-key/setup digest.
