...
- compact batch-STARK L2 over fast statement-bound L1: the current committed
  ...
  **126,862 bytes** compact wrapper / **125,979 bytes** body but raises cached
  ...
  digest. The remaining gaps are Hoon verifier wiring, production expected
  digest configuration, and the measured time reduction.
...
2. The compact final-layer batch-STARK L2 path is the committed primary
   ...
   expected digest. It is not yet wired through the Hoon verifier path and does
   not yet have production expected-digest configuration.
