...
- full `ai-pow-zk` composite-verifier integrated-LogUp polynomial NPO
  ...
  **36.590s** proving to **198,287 bytes** / **33.277s** proving, with compact
  ...
  production route now has to reduce the packed support FRI leaf payload itself
  by roughly **100 KiB** and then fold that leaner packed Tip5 binding into the
  merged value-bridge theorem. Additional prelude commitment caching,
  transcript sharing, and low-level quotient-loop cleanup alone will not make
  this candidate meet **150 KiB** or **30s total**.
