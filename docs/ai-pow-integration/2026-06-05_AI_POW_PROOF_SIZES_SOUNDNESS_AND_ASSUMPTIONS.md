...
- compact batch-STARK L2 over fast statement-bound L1: the current committed
  ...
  **126,862 bytes** compact wrapper / **125,979 bytes** body but raises cached
  ...
  and the uncached round-trip wall is **58.603s**. The remaining gap is
  bridge/miner/Hoon wiring plus production-pinned expected digest selection and
  the measured time reduction.
...
2. The compact final-layer batch-STARK L2 path is the committed primary
   ...
   noun path as canonical compact bytes. It is not yet wired through the Hoon
   verifier path and does not yet have a production-pinned expected
...
builder; the large checkpoint object remains a fallback/regression artifact.
