...
- compact batch-STARK L2 over fast statement-bound L1: the current committed
  ...
  measures **44.031s** cached serial L1+L2 per-attempt proving with both prep
  stages cached, so the route is not yet production-complete. The latest L2
  cached proving alone is **28.726s**, now under the relaxed time gate if
  counted without L1. A verified table-packing sweep finds a smaller L2 reserve
  at **126,862 bytes** compact wrapper / **125,979 bytes** metadata-free body
  with `alu_lanes=2`, but cached L2 proving rises to **30.801s** and cached
  serial L1+L2 remains about **46.469s**. Lower ALU lanes are therefore a size
  knob, not the time route. The prior
