...
| Packed AIR+LogUp selected-to-packed-trace shared theorem checkpoint | Negative diagnostic; proves packed Tip5 AIR algebra, packed byte-table LogUp, and lane-selector selected NPO-value to packed trace-lane binding in one transcript with one shared packed trace opening. It is sound for those included relations, but it is not a viable production proof shape. | On the actual PROD composite verifier relation, the shared theorem verifies at `273,113` bytes / `266.7 KiB`: compact FRI `256,261` bytes, selected lookup opening `1,700` bytes, packed trace opening `9,985` bytes, AIR quotient opening `42` bytes, LogUp table opening `22` bytes, LogUp accumulator opening `1,846` bytes, LogUp quotient opening `41` bytes, selected/packed bridge accumulator openings `732 + 729` bytes, and selected/packed bridge quotient openings `42 + 41` bytes. It uses AIR quotient rows `65,536`, LogUp quotient rows `65,536`, selected bridge quotient rows `65,536`, and packed bridge quotient rows `32,768`. Prove `36.590s`, verify `14.378s`, total diagnostic wall `98.387s`. This confirms that sharing the packed trace opening is not sufficient; the production route needs algebraic quotient/domain reduction before folding Tip5 binding into the merged value-bridge theorem. | `RUSTFLAGS="-C target-cpu=native" cargo test -p ai-pow-zk --release --features recursion terminal_packed_tip5_air_logup_selected_trace_bridge_candidate_for_prod_baseline_measures -- --ignored --nocapture`, 2026-06-06 |
...
- full `ai-pow-zk` composite-verifier integrated-LogUp polynomial NPO
  ...
  **7.600s** prelude construction, and **14.435s** primitive proving before the
  ...
  selected-trace theorem also verifies, but it measures **273,113 bytes** with
  **36.590s** proving and **256,261 bytes** of compact FRI, so sharing the
  full packed trace opening is not sufficient. These are sound checkpoints,
  not appendable production proof bodies; the production route now has to
  reduce committed domains/quotients and fold a leaner packed Tip5 binding into
  the merged value-bridge theorem.
