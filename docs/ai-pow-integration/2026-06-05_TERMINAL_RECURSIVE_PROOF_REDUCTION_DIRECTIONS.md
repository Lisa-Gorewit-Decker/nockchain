...
- The compact L2 prep/cache is no longer test-only. `ai-pow-zk` now exposes
  ...
  and Pearl-compatible compact builders. The cache owns verifier-circuit
  targets, AIR setup, preprocessed prover data, and table-prover registration
  for a fixed L1 proof shape, shares verifier-owned setup with the returned
  verifier context, and rejects stale L1 metadata/setup before proving. The
  compact run now returns freshly-built L2 setup as an optional prover cache
  when it did not use a cache, and the production Pearl-compatible miner
  lazily stores that cache after the first compact certificate so later solved
  tickets in the same process can skip repeat L2 setup when the L1 shape still
  matches. If the stale-cache guard rejects reuse, the bridge discards that
...
- Finish canonical production cache policy for the compact prover cache. The
  production miner now has lazy first-certificate cache warm-up and the Rust API
  guards stale setup, but proactive warm-up, operator lifecycle controls, and
  Hoon/verifier integration for the pinned digest remain open.
