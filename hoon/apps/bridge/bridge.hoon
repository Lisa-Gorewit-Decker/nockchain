...
++  bridge
  ...
  ++  handle-cause
    ...
    ~&  %handle-cause
    ?^  stop.state
      =+  base-hash-b58=(to-b58:hash:t base-hash.u.stop.state)
      =+  nock-hash-b58=(to-b58:hash:t nock-hash.u.stop.state)
      =/  msg=@t
        ;:  (cury cat 3)
    ...
    ?:  ?&  ?=(^ base-hold.hash-state.state)
        ...
        ==
      [[%0 %stop 'fatal: holds exist for both hashchains' [last-base-blocks last-nock-block]:hash-state.state]~ state]
    ...
    =;  result
      ?-    -.result
        ...
        %-  (slog p.result)
        [[%0 %stop msg [last-base-blocks last-nock-block]:hash-state.state]~ state]
     ::
          %&
        p.result
      ==
  ...
  ++  evaluate-base-call
    ...
    ~&  %evaluate-base-call
    =+  old-state=state
    =/  old-last-blocks  [last-base-blocks last-nock-block]:hash-state.old-state
    =/  template  |=(msg=@t [%0 %stop msg old-last-blocks])
    |-
    ?~  proposal
      [~ old-state]
