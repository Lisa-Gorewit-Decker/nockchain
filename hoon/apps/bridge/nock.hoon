...
++  incoming-nockchain-block
  ...
  =/  old-state  state
  =/  old-last-blocks  [last-base-blocks last-nock-block]:hash-state.state
  ...
  ?:  !=(tx-ids.block.nockchain-block ~(key z-by txs.nockchain-block))
    [[%0 %stop 'tx-ids mismatch txs in nockchain block' old-last-blocks]~ old-state]
  ...
  ?^  stop=(validate-nockchain-page-sequence block.nockchain-block)
    [[%0 %stop u.stop old-last-blocks]~ old-state]
  ...
  ?-    -.process-block
    ...
    ?-  -.process-fail
      %stop  [[%0 %stop msg.process-fail old-last-blocks]~ old-state]
...
++  process-nockchain-block
  ...
  ++  process-nock-txs
    ...
    ?:  (is-bridge-deposit-tx tx)
      ...
      $(tx-list t.tx-list)
    ?:  (is-bridge-withdrawal-tx tx bridge-lock-root.state)
  ...
  ++  extract-deposit-intent
    ...
    ~?  =(~ recipient)
      'Encountered malformed evm recipient address. Deposited nocks will remain in bridge nockchain wallet.'
    ~&  recipient+recipient
