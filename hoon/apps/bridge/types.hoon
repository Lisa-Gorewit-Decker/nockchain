...
++  blist
  ...
  ++  from-atom
    ...
    [r $(n q)]
  ::
  ::  +to-atom: convert a based list back to an atom (inverse of +to)
  ++  to-atom
    |=  l=form
    ^-  @
    ?~  l  0
...
+$  bridge-state
  $:  %0
      ...
      stop=(unit [=base-hash =nock-hash])                   ::  flag to stop the bridge. populated with last known good block hashes if stop is true.
  ==
::
...
++  deposit-settlement
  ...
  ++  hashable
    ...
    :*  (hashable:beid beid.form)
        ...
        (hashable:blist (from-atom:blist dest.form))
  ...
  +$  form
    ...
    $:  %0
      $%  [%base-call base-call]
          ...
          [%grpc grpc-effect]
          [%stop reason=cord last=[=base-hash nock-hash]]
      ==
    ==
  ::
