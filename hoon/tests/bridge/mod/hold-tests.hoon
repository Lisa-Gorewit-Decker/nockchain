...
++  test-base-deposit-settlement-commits-only-after-ack
  ...
  ==
::  Explicit repair rewinds stale pending state for watcher replay.
++  test-repair-pending-base-block-commit-reprocesses-after-clear
  ^-  tang
  =/  state=bridge-state  *bridge-state
  =/  constants=bridge-constants  (small-constants:hel 1 10 0)
  =.  constants.state  constants
  ...
  =/  as-of=nock-hash  (hash:nock-block nb)
  =/  event-id=beid  (from-atom:blist 40)
  =/  settlement-event=base-event
    :*  (to-atom:blist event-id)
        [%deposit-processed dep-tx dep-name dest amount height.nb as-of 12]
    ==
  =/  raw=raw-base-blocks:cause
    :~  [10 0x40 0x0 ~[settlement-event]]
    ==
  =/  base  ~(. base-lib state)
  =/  blocks=base-blocks  (cook-base-blocks:base raw)
  =/  blocks-hash=base-hash  (hash:base-blocks blocks)
  ...
  =/  stage-result=process-result
    (stage-base-blocks:base blocks pending)
  ?>  ?=(%& -.stage-result)
  =/  staged=bridge-state  +.stage-result
  =/  ack=base-block-commit-ack
    [blocks-hash first-height.pending last-height.pending]
  =/  base-staged  ~(. base-lib staged)
  =/  [stop-effects=(list effect) stopped=bridge-state]
    (commit-base-block-withdrawals:base-staged ack)
  =/  base-stopped  ~(. base-lib stopped)
  =/  [repair-effects=(list effect) held=bridge-state]
    (repair-pending-base-block-commit:base-stopped ack)
  ?>  ?=(^ base-hold.hash-state.held)
  =/  hold=[hash=hash:t height=@]  u.base-hold.hash-state.held
  =/  [added-block=nock-block nock-state=bridge-state]
    (add-nockchain-blocks:hel held deposits *(z-map nname:t withdrawal-settlement))
  ?>  =(as-of (hash:nock-block added-block))
  =.  base-hold.hash-state.nock-state  ~
  =.  unsettled-deposits.hash-state.nock-state
    (~(put z-bi unsettled-deposits.hash-state.nock-state) as-of dep-name dep)
  =/  base-ready  ~(. base-lib nock-state)
  =/  [replay-effects=(list effect) staged-again=bridge-state]
    (incoming-base-blocks:base-ready [raw [~ 0 0x0 *@da]])
  ?~  replay-effects
    ~|('expected replayed base-block-withdrawals-pending effect' !!)
  ?>  ?=([%0 %base-block-withdrawals-pending *] i.replay-effects)
  =/  replay-pending=pending-base-block-withdrawals  pending.i.replay-effects
  =/  replay-ack=base-block-commit-ack
    [blocks-hash.replay-pending first-height.replay-pending last-height.replay-pending]
  =/  base-staged-again  ~(. base-lib staged-again)
  =/  [commit-effects=(list effect) committed=bridge-state]
    (commit-base-block-withdrawals:base-staged-again replay-ack)
  =/  has-unsettled=?
    (~(has z-bi unsettled-deposits.hash-state.committed) as-of dep-name)
  ;:  weld
    (expect !>((has-stop-effect stop-effects)))
  ::
    (expect !>(?=(^ pending-base-block-commit.hash-state.stopped)))
  ::
    (expect !>(?=(~ base-hold.hash-state.stopped)))
  ...
  ::
    %+  expect-eq
      !>(%.n)
    !>((has-stop-effect repair-effects))
  ::
    %+  expect-eq
      !>(as-of)
    !>(hash.hold)
  ...
  ::
    %+  expect-eq
      !>(~)
    !>(withdrawals.replay-pending)
  ::
    %+  expect-eq
      !>(blocks-hash)
    !>(blocks-hash.replay-pending)
  ::
    (expect !>(?=(^ pending-base-block-commit.hash-state.staged-again)))
  ::
    %+  expect-eq
      !>(~)
    !>(commit-effects)
  ...
  ::
    (expect !>((~(has z-by base-hashchain.hash-state.committed) blocks-hash)))
  ::
    (expect !>(!has-unsettled))
  ==
::  Repair missing as-of stops instead of creating simultaneous holds.
++  test-repair-pending-base-block-commit-stops-before-both-holds
  ^-  tang
  =/  state=bridge-state  *bridge-state
  =/  constants=bridge-constants  (small-constants:hel 1 10 0)
  =.  constants.state  constants
  ...
  =/  dep-name=nname:t  *nname:t
  =/  dep-tx=tx-id:t  *tx-id:t
  =/  dest=base-addr  0xcccc
  =/  event-id=beid  (from-atom:blist 41)
  =/  unknown-as-of=nock-hash  [0xc 0xc 0xc 0xc 0xc]
  =/  settlement-event=base-event
    :*  (to-atom:blist event-id)
        [%deposit-processed dep-tx dep-name dest 1.000.000 400 unknown-as-of 17]
    ==
  =/  raw=raw-base-blocks:cause
    :~  [10 0x41 0x0 ~[settlement-event]]
    ==
  =/  base  ~(. base-lib state)
  =/  blocks=base-blocks  (cook-base-blocks:base raw)
  =/  blocks-hash=base-hash  (hash:base-blocks blocks)
  ...
  ?>  ?=(%& -.stage-result)
  =/  staged=bridge-state  +.stage-result
  =/  nock-hold-value=[hash=base-hash height=@]  [[0xd 0xd 0xd 0xd 0xd] 88]
  =.  nock-hold.hash-state.staged  `nock-hold-value
  =/  ack=base-block-commit-ack
    [blocks-hash first-height.pending last-height.pending]
  =/  base-staged  ~(. base-lib staged)
  =/  [effects=(list effect) stopped=bridge-state]
    (repair-pending-base-block-commit:base-staged ack)
  ?>  ?=(^ pending-base-block-commit.hash-state.stopped)
  =/  stopped-pending=pending-base-block-commit-data
    u.pending-base-block-commit.hash-state.stopped
  ?>  ?=(^ nock-hold.hash-state.stopped)
  =/  stopped-nock-hold=[hash=base-hash height=@]
    u.nock-hold.hash-state.stopped
  ;:  weld
