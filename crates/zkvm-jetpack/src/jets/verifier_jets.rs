...
use nockvm::noun::{Atom, Cell, IndirectAtom, Noun, NounSpace};
...
fn mpeval<F: Fops>(
    ...
    */
    ...
    let mut mp = HoonMapIter::new(mp, space);

    mp.try_fold(F::zero(), |acc, n| {
        let [k, v] = n.uncell(space)?;

        let Ok(k) = BPolySlice::try_from(k, space) else {
            return Err(BAIL_FAIL);
        };
        let v = Belt::from_noun(v, space)?;
        // =/  coeff=@ux  (lift-op v)
        let coeff = F::lift(v);
        // ?:  =(init-zero coeff)
        if coeff == F::zero() {
