    ...
    SymbolicExpressionExt<Val<SC>, SC::Challenge>: Algebra<SC::Challenge>,
{
    ...
    for i in 0..n_instances {
        transcript.observe_instance_binding(
            log_ext_degrees[i], log_degrees[i], widths[i], num_quotient_chunks[i],
        );
    }
        ...
        .for_each(|((i, inst), ext_domain)| {
            if !all_lookups[i].is_empty() {
                ...
                #[cfg(debug_assertions)]
                {
                    ...
                    check_constraints(
                        inst.air, inst.trace, &preprocessed_trace, &generated_perm,
                        &challenges_per_instance[i], &perm_vals, &inst.public_values,
    ...
    // Stable mapping from AIR instance index to the compacted permutation
    // matrix index used by the PCS. The quotient loop below can then run per
    // instance in parallel while preserving the same matrix selection the
    // original serial `perm_counter` used.
    let mut next_perm_index = 0usize;
    let permutation_trace_indices = all_lookups
        .iter()
        .map(|lookups| {
            if lookups.is_empty() {
                None
            } else {
                let index = next_perm_index;
                next_perm_index += 1;
                Some(index)
            }
        })
        .collect::<Vec<_>>();
    let permutation_prover_data = permutation_commit_and_data.as_ref().map(|(_, data)| data);
    let preprocessed_matrix_indices = common.preprocessed.as_ref().map(|global| {
        global
            .instances
            .iter()
            .map(|instance| instance.as_ref().map(|meta| meta.matrix_index))
            .collect::<Vec<_>>()
    });
    let preprocessed_prover_data = preprocessed_matrix_indices.as_ref().map(|_| {
        prover_data
            .prover_only
    ...
    // Compute per-instance quotient chunks independently, then concatenate in
    // instance order before the single quotient commitment. This preserves the
    // transcript and proof layout while removing the serial per-AIR quotient
    // scheduling bottleneck.
    let quotient_chunks_by_instance = (0..n_instances)
        .into_par_iter()
        .map(|i| {
            ...
            debug_assert!(
                airs[i].num_constraints().is_none_or(|n| {
                    n == get_symbolic_constraints(
                        airs[i], sym_layout, all_lookups[i], &lookup_gadget,
                    )
                    .0
                    .len()
                }),
            ...
            let permutation_on_quotient_domain = permutation_prover_data
                .filter(|_| !all_lookups[i].is_empty())
                .map(|perm_data| {
                    pcs.get_evaluations_on_domain(
                        perm_data,
                        permutation_trace_indices[i]
                            .expect("lookup instance must have a permutation matrix index"),
                        quotient_domain,
                    )
                });

            // Evaluate preprocessed columns on the quotient domain (if present).
            let preprocessed_on_quotient_domain = preprocessed_matrix_indices
                .as_ref()
                .and_then(|indices| indices[i])
                .map(|matrix_index| {
                    pcs.get_evaluations_on_domain_no_random(
                        preprocessed_prover_data.expect(
    ...
    for (chunk_domains, ldes) in quotient_chunks_by_instance {
