...
fn report(label: &str, knobs: FriKnobs, cap: usize, d4: bool) {
    ...
    let result = if d4 {
        tip5_d4::build_l1(knobs, cap, &full)
    } else {
        tip5_d5::build_l1(knobs, cap, &full)
    };
    match result {
...
fn l1_combined_optimization_sweep() {
    ...
    report(
        "d4 + lb=4 nq=20 + mla=3 lfp=2 + cap=3",
        FriKnobs {
            log_blowup: 4,
            num_queries: 20,
            max_log_arity: 3,
    ...
    report(
        "d4 + lb=4 nq=20 pow=0+0 + mla=3 lfp=2 + cap=3",
        FriKnobs {
            log_blowup: 4,
            num_queries: 20,
            commit_pow: 0,
