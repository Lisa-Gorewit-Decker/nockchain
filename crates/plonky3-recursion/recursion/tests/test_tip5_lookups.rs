...
use p3_circuit::ops::{
    Tip5Config, Tip5Goldilocks, Tip5PermCall, generate_tip5_trace, perm_private_data,
};
...
fn prove_tip5_mmcs_direction_one() {
    let mut builder: CircuitBuilder<F> = CircuitBuilder::new();
    let tip5_config = Tip5Config::GOLDILOCKS_W16;
    ...
    perm_inputs[0] = Some(input0);
    perm_inputs[1] = Some(input1);
    let (_leaf_op, leaf_outputs) = builder
        .add_tip5_perm(&Tip5PermCall {
            config: tip5_config,
            new_start: true,
            inputs: perm_inputs,
    ...
    builder.assert_bool(mmcs_bit);
    let mut mmcs_inputs = vec![None; 16];
    for i in 0..5 {
        mmcs_inputs[i] = leaf_outputs[i];
    }
    let (mmcs_op, mmcs_outputs) = builder
    ...
    for out in mmcs_outputs.iter().take(5) {
        let expected = builder.public_input();
        let diff = builder.sub(out.unwrap(), expected);
        builder.assert_zero(diff);
    }
    ...
    let mut runner = circuit.runner();
    let in0 = F::from_u64(0x1357_9bdf_2468_ace0);
    let in1 = F::from_u64(0xfdb9_7531_eca8_6420);
    let mut leaf_state = [F::ZERO; STATE_SIZE];
    leaf_state[0] = in0;
    leaf_state[1] = in1;
    let leaf_image = tip5_native(&leaf_state);
    ...
    let mut mmcs_state = [F::ZERO; STATE_SIZE];
    mmcs_state[..5].copy_from_slice(&sibling);
    mmcs_state[5..10].copy_from_slice(&leaf_image[..5]);
    let mmcs_image = tip5_native(&mmcs_state);

    let mut pis = vec![in0, in1, F::ONE];
    pis.extend_from_slice(&mmcs_image[..5]);
    runner.set_public_inputs(&pis).unwrap();
    runner
        .set_private_data(mmcs_op, perm_private_data(tip5_config, sibling))
        .unwrap();
    let traces = runner.run().unwrap();
    ...
    prover
        ...
        .expect("direction-bit-1 Tip5 MMCS row should satisfy WitnessChecks");
}

#[test]
fn test_tip5_mmcs_direction_one_ctl_lookups() {
    prove_tip5_mmcs_direction_one();
}
