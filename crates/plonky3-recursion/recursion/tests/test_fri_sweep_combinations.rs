...
fn build_layer0_verifier_circuit_with_knobs(inner: InnerKnobs, tamper: bool) -> BuiltLayer0Circuit {
...
fn l2_over_l1_with_knobs(
    ...
    let v_traces = runner
        .run()
        .map_err(|e| format!("[{label}] L2 runner().run(): {e:?}"))?;
    ...
    v_prover
        ...
        .map_err(|e| format!("[{label}] L2 verify_all_tables: {e:?}"))?;
...
fn fri_sweep_inner_outer_combinations() {
    ...
    eprintln!(
        "All points: 5-round Tip5, paper-faithful digest=5, ≥80-bit unconditional Johnson.\n"
    );
    ...
    for point in &sweep {
        ...
        let label = format!(
            "{} (inner {}b / outer {}b)",
            point.label, inner_bits, outer_bits
        );
        eprintln!("\n>>> Running {label}");
