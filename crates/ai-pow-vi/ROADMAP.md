...
| `oracle/streaming_merkle.py` + `streaming_writer.py` + `quantize_streaming.py` + `gguf_reader.iter_tensors` | 2.15 | `0bf1b57` | Streaming GGUF→Model pipeline: stack-of-subtrees Merkle (O(log n) memory), per-tensor on-demand dequant (peak ≈ largest single tensor, ~2 GB for 19 GB GGUFs vs ~64 GB for the materializing path). Byte-equal to the reference path for all 5 layer flavors. Unblocks real-model conversion on a 16 GB laptop. |
| Streaming converter: all 5 layer flavors | 2.15.1 | _this commit_ | Per-block-kind helpers + metadata builders for DeltaNet, Gemma, QwenStandard, QwenHybridSsm. Each helper produces byte-identical output to the corresponding `streaming_writer._stream_layer_weights` arm (verified by 6 parity tests via a `MockGgufStream` wrapper). Default no-op gamma (all-1.0 f32 → all-127 i8) for arch-required tensors that real GGUFs may omit (e.g. Qwen 3.6 hybrid QK norm). |

Test count: 197 unit + 18 pins + 7 oracle cross-impl + 4 qwen-mini E2E + 3 quantized-synthetic E2E + 4 gemma-mini E2E + 4 qwen-hybrid-mini E2E + 4 oracle-multi-arch + 5 oracle-arch (Python) + 5 vi-eval bin + 11 streaming-merkle + 4 streaming-writer + 3 streaming-quantize + 6 streaming-quantize-all-archs + 1 gated real-model, all green on aarch64.
