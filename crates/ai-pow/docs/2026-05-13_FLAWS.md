...
Evaluated against:
- Pearl Whitepaper — concrete INT8 L1 PoUW design.
- Komargodski & Weinstein, *"Proofs of Useful Work from Arbitrary Matrix Multiplication"* (arXiv:2504.09971) — the **cuPOW** scheme, esp. Algorithm 6.4 and §6.5.
...
- Pearl §4.5 (Alg. 4): per-tile state `M ∈ {0,1}^{512}` (sixteen `int32`s) is updated **at every k-axis accumulation step**. At step ℓ < ⌊k/r⌋, the prover computes the int32 XOR of all entries in the current accumulator tile `C_blk`, then folds it in with rotation:
  ...
  ```
  Pearl's final hardness check is `BLAKE3(M, key=s_A) ≤ 2^(256-b) · r · t_m · t_n`.
  In Nockchain after the 2026-05-31 grinding repair, the nonce is also folded
  into the attempt before `s_A` and the final jackpot key is
  `pow_key_for_nonce(s_A, nonce)`, not raw `s_A`.
...
- **Per-nonce search**: Superseded by the 2026-05-31 grinding fix. Pearl proper has no nonce; in this crate the Nockchain nonce is now part of the Pearl attempt state before `κ`, matrix commitments, noise seeds, and matmul-derived tile states are computed. `pow_key = derive_key("pow-key", s_A ‖ nonce)` remains only a final-hash domain key, not a cheap retry mechanism.
...
### Verifying the fixes
- `cargo test -p ai-pow` — historical count; current tests include nonce-grinding regressions for nonce-bound commitments/noise/tile states.
