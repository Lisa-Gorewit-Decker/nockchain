...
///
/// This compares two nouns for structural equality without modifying them
/// (unlike unifying equality which may merge identical substructures).
/// This is suitable for use with Pma since we don't need to allocate
/// temporary structures during comparison.
///
...
/// Compute the mug (hash) of a noun, caching results.
///
/// This traverses the noun and computes mugs bottom-up, caching them
/// in the noun metadata for future use.
pub fn compute_mug(noun: Noun) -> u32 {
    use crate::mug::{calc_atom_mug_u32, calc_cell_mug_u32, set_mug};
            ...
            if allocated.get_cached_mug().is_none() {
                match allocated.as_either() {
                    ...
                    Right(cell) => match (get_mug(cell.head()), get_mug(cell.tail())) {
                        ...
                        _ => {
                            // Children not yet mugged - revisit after children
                            stack.push(noun);
    ...
    get_mug(noun).expect("Noun should have a mug once mugged")
}
