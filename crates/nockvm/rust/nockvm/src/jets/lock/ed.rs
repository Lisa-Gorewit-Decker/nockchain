...
use crate::jets::util::{slot_with_arena, BAIL_EXIT};
...
pub fn jet_puck(context: &mut Context, subject: Noun) -> Result {
    ...
    let sed = slot_with_arena(subject, 6, arena)?.as_atom()?;

    let sed_len = met(3, arena, sed);
    if sed_len > 32 {
    ...
    unsafe {
        ...
        sed_bytes[0..sed_len].copy_from_slice(&(sed.as_ne_bytes_with_arena(arena))[0..sed_len]);

        let (mut pub_ida, pub_key) = IndirectAtom::new_raw_mut_bytearray::<32, NockStack>(stack);
        ac_ed_puck(sed_bytes, pub_key);

        Ok(pub_ida.normalize_as_atom_with_arena(arena).as_noun())
    }
}
...
pub fn jet_shar(context: &mut Context, subject: Noun) -> Result {
    let arena = &*context.arena;
    let stack = &mut context.stack;
    let pub_key = slot_with_arena(subject, 12, arena)?.as_atom()?;
    let sec_key = slot_with_arena(subject, 13, arena)?.as_atom()?;

    if met(3, arena, sec_key) > 32 {
        // sek is size checked by +puck via +suck
        return Err(BAIL_EXIT);
    }
    if met(3, arena, pub_key) > 32 {
    ...
    unsafe {
        ...
        let pub_bytes = pub_key.as_ne_bytes_with_arena(arena);
        let sec_bytes = sec_key.as_ne_bytes_with_arena(arena);
        ...
        Ok(shar_ida.normalize_as_atom_with_arena(arena).as_noun())
    }
}
...
pub fn jet_sign(context: &mut Context, subject: Noun) -> Result {
    let arena = &*context.arena;
    let stack = &mut context.stack;
    let msg = slot_with_arena(subject, 12, arena)?.as_atom()?;
    let sed = slot_with_arena(subject, 13, arena)?.as_atom()?;

    unsafe {
        let sed_bytes = sed.as_ne_bytes_with_arena(arena);
        ...
        let msg_len = met(3, arena, msg);
        if msg_len > 0 {
            let (_msg_ida, message) = IndirectAtom::new_raw_mut_bytes(stack, msg_len);
            message.copy_from_slice(&msg.as_ne_bytes_with_arena(arena)[0..msg_len]);
        ...
        sig.reverse();
        Ok(sig_ida.normalize_as_atom_with_arena(arena).as_noun())
    }
}

pub fn jet_veri(context: &mut Context, subject: Noun) -> Result {
    let arena = &*context.arena;
    let sig = slot_with_arena(subject, 12, arena)?.as_atom()?;
    let msg = slot_with_arena(subject, 26, arena)?.as_atom()?;
    let puk = slot_with_arena(subject, 27, arena)?.as_atom()?;

    // Both are size checked by Hoon, but without crashing
    let sig_bytes = sig.as_ne_bytes_with_arena(arena);
    if sig_bytes.len() > 64 {
        return Ok(NO);
    };
    ...
    let pub_bytes = puk.as_ne_bytes_with_arena(arena);
    if pub_bytes.len() > 32 {
        return Ok(NO);
    };
    ...
    let message = &(msg.as_ne_bytes_with_arena(arena))[0..met(3, arena, msg)]; // drop trailing zeros
