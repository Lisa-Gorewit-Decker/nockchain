...
pub fn jet_shas(context: &mut Context, subject: Noun) -> Result {
    ...
    unsafe {
        ...
        let sal_bytes = &(sal.as_ne_bytes(&space))[0..met(3, sal, &space)]; // drop trailing zeros
        ...
        if msg_len > 0 {
            let msg_bytes = &(ruz.as_ne_bytes(&space))[0..msg_len];
...
pub fn jet_shay(context: &mut Context, subject: Noun) -> Result {
    ...
    unsafe {
        ...
        if length == 0 {
            ...
            msg.copy_from_slice(&(ruz.as_ne_bytes(&space))[0..length]);
            ac_shay(msg, out);
        } else {
            let msg_bytes = &(ruz.as_ne_bytes(&space))[0..msg_len];
            let (mut _msg_ida, msg) = IndirectAtom::new_raw_mut_bytes(stack, length);
