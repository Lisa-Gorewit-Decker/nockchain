...
pub fn z_map_put<A: NounAllocator, H: TipHasher>(
    ...
    if unsafe { a.raw_equals(&D(0)) } {
        ...
        if unsafe { stack.equals(b, &mut anp) } {
        ...
        } else if gor_tip(stack, b, &mut anp, hasher, arena)? {
            let d = z_map_put(stack, &mut al, b, c, hasher, arena)?;
            let [dn, dl, dr] = d.uncell()?;
            let [mut dnp, _dnq] = dn.uncell()?;
            if mor_tip(stack, &mut anp, &mut dnp, hasher, arena)? {
                Ok(T(stack, &[an, d, ar]))
            } else {
            ...
            let d = z_map_put(stack, &mut ar, b, c, hasher, arena)?;
            let [dn, dl, dr] = d.uncell()?;
            let [mut dnp, _dnq] = dn.uncell()?;
            if mor_tip(stack, &mut anp, &mut dnp, hasher, arena)? {
                Ok(T(stack, &[an, al, d]))
