// Pearl-compatible BLAKE3 keyed-mode AIR (M10.1b).
//
// Vendored from Plonky3/Plonky3 @ af65376 (`blake3-air/src/air.rs`).
// The constraint logic in this file is **unmodified** from upstream —
// it already references `local.flags` correctly as `initial_row_3[3]`,
// so once the trace generator populates `flags` (see `generation.rs`)
// the AIR proves real BLAKE3 keyed-mode compression. Only the imports,
// the struct name, and the unused random-input helper have been
// changed.
...
use super::columns::{Blake3Cols, Blake3State, FullRound, QuarterRound, NUM_BLAKE3_COLS};
use super::constants::{permute, BITS_PER_LIMB, IV};

/// Pearl-compatible BLAKE3 compression-function AIR. Identical
/// constraints to upstream `p3_blake3_air::Blake3KeyedAir`; only the trace
/// generator was patched to populate `flags` so this AIR can prove
/// real BLAKE3 keyed-mode compression (not the flags-zero variant).
#[derive(Debug, Default)]
pub struct Blake3KeyedAir {}
...
impl<F> BaseAir<F> for Blake3KeyedAir {
    fn width(&self) -> usize {
        NUM_BLAKE3_COLS
    }
...
impl<AB: AirBuilder> Air<AB> for Blake3KeyedAir {
    #[inline]
    fn eval(&self, builder: &mut AB) {
        let main = builder.main();
        let local: &Blake3Cols<AB::Var> = main.current_slice().borrow();

        let initial_row_3 = [local.counter_low, local.counter_hi, local.block_len, local.flags];
        ...
        // Round 2:
        self.verify_round(
            builder, &local.full_rounds[0].state_output, &local.full_rounds[1], &m_values,
        );
        ...
        // Round 3:
        self.verify_round(
            builder, &local.full_rounds[1].state_output, &local.full_rounds[2], &m_values,
        );
        ...
        // Round 4:
        self.verify_round(
            builder, &local.full_rounds[2].state_output, &local.full_rounds[3], &m_values,
        );
        ...
        // Round 5:
        self.verify_round(
            builder, &local.full_rounds[3].state_output, &local.full_rounds[4], &m_values,
        );
        ...
        // Round 6:
        self.verify_round(
            builder, &local.full_rounds[4].state_output, &local.full_rounds[5], &m_values,
        );
        ...
        // Round 7:
        self.verify_round(
            builder, &local.full_rounds[5].state_output, &local.full_rounds[6], &m_values,
        );
        ...
        for (out_bits, left_words, right_bits) in izip!(
            local.outputs[0], local.full_rounds[6].state_output.row0, local.final_round_helpers
        ) {
        ...
        for (out_bits, left_bits, right_bits) in izip!(
            local.outputs[1], local.full_rounds[6].state_output.row1,
            local.full_rounds[6].state_output.row3
        ) {
            for (out_bit, left_bit, right_bit) in izip!(out_bits, left_bits, right_bits) {
                let left_expr = <AB::Var as Into<AB::Expr>>::into(left_bit);
                let right_expr = <AB::Var as Into<AB::Expr>>::into(right_bit);
                builder.assert_eq(out_bit, left_expr.xor(&right_expr));
            }
        }
        ...
        for (out_bits, left_bits, right_bits) in
            izip!(local.outputs[2], local.chaining_values[0], local.final_round_helpers)
        {
            for (out_bit, left_bit, right_bit) in izip!(out_bits, left_bits, right_bits) {
                let left_expr = <AB::Var as Into<AB::Expr>>::into(left_bit);
                let right_expr = <AB::Var as Into<AB::Expr>>::into(right_bit);
        ...
        for (out_bits, left_bits, right_bits) in izip!(
            local.outputs[3], local.chaining_values[1], local.full_rounds[6].state_output.row3
        ) {
            for (out_bit, left_bit, right_bit) in izip!(out_bits, left_bits, right_bits) {
                let left_expr = <AB::Var as Into<AB::Expr>>::into(left_bit);
                let right_expr = <AB::Var as Into<AB::Expr>>::into(right_bit);
