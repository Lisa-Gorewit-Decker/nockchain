...
| **M3** | landed | `public::PublicInputs` ↔ `Vec<Goldilocks>` codec (historical 42-element shape; superseded by the current 60-element `CompositePublicInputs` ABI) | 11 |
...
Current ABI note: this historical section describes the M10-era
42-element vector. The active `CompositePublicInputs` layout is
60 field elements:
`cumsum(4) + jackpot(16) + hash_a(8) + hash_b(8) + job_key(8) +
commitment_hash(8) + hash_jackpot(8)`. See
`src/composite_public.rs`.

At the time, `lib::prove` / `lib::verify` called
`public_inputs.to_field_elements()` and passed the then-current
42-element vector to `p3_uni_stark::{prove, verify}` as the `&pis`
argument. The composite AIR declared
`num_public_values = NUM_PUBLIC_INPUTS = 42` so Plonky3 absorbed the
PIs into the Fiat-Shamir challenger. This paragraph is historical; the
active ABI is the 60-element shape noted above.
