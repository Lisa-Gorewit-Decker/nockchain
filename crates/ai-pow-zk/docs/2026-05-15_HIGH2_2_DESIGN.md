...
> `15ba9a3`.
> **Authoritative neighbors:** `2026-05-15_ZKP_SECURITY_REPORT.md` (HIGH-2
> section + Bottom line), `2026-05-15_GAP_AUDIT.md` (post-CRIT-1 + keystone
...
(`HASH_JACKPOT = BLAKE3(M, key=pow_key_for_nonce(s_a, nonce))`,
byte-equivalent to the plain miner), C2 checks difficulty. Full
`cargo test -p ai-pow --features zk` green. The pre-existing latent JackpotChip bug
...
state M, key=pow_key_for_nonce(s_a, nonce))` for a tile derived
from the **committed** matrices — i.e. the digest the plain PoW
search actually cleared — and therefore able to win at a **real**
difficulty target.
...
```
hasher = blake3::Hasher::new_keyed(pow_key)   // Nockchain: pow_key_for_nonce(s_a, nonce)
...
`2026-05-15_ZKP_SECURITY_REPORT.md` when HIGH-2.2 closes.**
...
> **⚠️ CORRECTION (2026-05-17) — `2026-05-17_M_S2_PEARL_EVALUATION.md` is
...
surface (`2026-05-17_G3_RECURSION_AUDIT.md` P0–P6). The §6(b) per-row
...
> `crates/ai-pow-zk/2026-05-17_G3_RECURSION_AGGREGATION.md`** (conceptual
...
> — see `crates/ai-pow-zk/2026-05-17_G3_RECURSION_AUDIT.md`, AUTHORITATIVE
...
cap. See `2026-05-17_M_S2_PEARL_EVALUATION.md`. G3 is DEFERRED per the γ
decision.]*
...
test-only / remove). See `2026-05-17_M_S2_PEARL_EVALUATION.md`.]*
...
6. Wire FoldChip + §4.D keystone; §7 end-to-end at real
   difficulty; doc + `2026-05-15_ZKP_SECURITY_REPORT.md` flip (note the
...
doc-commented with the obligation. See `2026-05-15_ZKP_SECURITY_REPORT.md`
§MED-3.
...
- `high2_2_byte_equiv_plain` — assert SNARK `HASH_JACKPOT` ==
  `TileState::keyed_hash(&ctx.pow_key())` from the plain solve,
  byte-for-byte.
...
> `2026-05-17_PRODUCTION_ROADMAP.md`** — the execution wrapper that ties
...
| **(deferred) G3a/G3b/G3c** — carry-vector segmentation + recursive *aggregation* | Matmul truth for per-tile `k` **beyond Pearl's `2¹⁶` cap** (strictly more ambitious than Pearl). Pursued **only if** a concrete load past the Pearl envelope is required. Design preserved in `2026-05-17_M_S2_G3AB_DESIGN.md`. | deferred (γ decision) |
...
- Keystone & soundness boundary: `2026-05-15_ZKP_SECURITY_REPORT.md`
  (HIGH-2 section + Bottom line), commit `15ba9a3`.
- Post-CRIT-1/keystone status: `2026-05-15_GAP_AUDIT.md`.
