...
- ◑ **Miner-chosen A,B with binding commitments.** M52 — landed
  at TEST_SMALL, see `2026-05-14_M52_MATRIX_BINDING.md`. Residual gap below.
...
(See `2026-05-14_ENGINEERING_REPORT.md` §6 + `2026-05-14_M52_MATRIX_BINDING.md` §7.)
...
or M12 lands. See `2026-05-14_ENGINEERING_REPORT.md` §11.
...
F1 harness + `scripts/profile_f1.sh` + `2026-05-15_PROFILING.md` (samply /
peak-RSS P2 / CI-bench P4) remain the instrumentation substrate.

**Bound non-vacuously on a real solve (zk_bridge):**
- **C1** — `JOB_KEY` = κ and `COMMITMENT_HASH` =
  `pow_key_for_nonce(s_a, nonce)` via
...
- **(a) Selector multiplexing** — `IS_HASH_JACKPOT` is the
  ...
  8 rows are a keyed BLAKE3 of `JACKPOT_MSG` (Nockchain key =
  `pow_key_for_nonce(s_a, nonce)`); row 7
...
- **(b) `verify_round` leading-boundary gate bug** — the deeper
  ...
  Full write-up + rationale: `2026-05-15_BLAKE3_CHIP_ROUND_GATE_BUG.md`
...
`BLAKE3(zeros, key=pow_key_for_nonce(s_a, nonce))` is a genuine
keyed digest and the binding constraint is fully exercised.
...
> `2026-05-15_ZKP_SECURITY_REPORT.md` is the authority and is updated to
> STATUS: RESOLVED.
>
...
> `2026-05-17_CANONICAL_PROGRAM_DESIGN.md` §7 is the authoritative status.
...
`≤2²²` + vertical-recursion certificate — `2026-05-17_M_S2_PEARL_EVALUATION.md`,
...
See `2026-05-19_C3_OUTER_CERT_DESIGN.md` §13.2/§14/§15,
`2026-05-17_PRODUCTION_ROADMAP.md` C3/M-S5b.]* *[Corrected: this previously
...
`2026-05-17_SEC_4C2_NOISE_BINDING_DESIGN.md` §8. Plus
7-round-Tip5 review, recursion (M12),
production-hardening (P1/P3/P5/P6).

---

## CSA deepening items routed 2026-05-20

The **Constraint Soundness Analysis** (CSA — landed 2026-05-20,
all 8 stages S0–S7;
`2026-05-20_CONSTRAINT_SOUNDNESS_ANALYSIS_DESIGN.md`) verified
every AIR + LogUp bus ≥80 unconditional bits and confirmed
every constraint family has ≥1 tamper test. Two items are
explicitly routed here as **deepening** (not gaps); they
extend coverage beyond the M-S6 audit baseline and are
post-audit follow-up if requested:

### 🔵 CSA-D1 — F3–F20 fine-grained per-FRI-fold-round direct tampers

Currently covered indirectly via
`c3_stage_a_l1_120bit_kat` + `c3_stage_b_l2_over_120bit_l1`
+ `c3_stage_c_sweep_120bit` (the composite outer-cert tests).
Direct per-fold-round tamper tests would add `crates/plonky3-recursion/recursion/tests/fri.rs`
variants for each fold-round commitment / opening / challenge
derivation. Effort: 1–2 days.

**Disposition.** Deferred-as-deepening; not blocking C4 audit.
The M-S6 audit baseline (every fold-round covered indirectly)
is satisfied. See `2026-05-20_CSA_S5_CROSS_AIR_TAMPER_TESTS.md`
§ 3.

### 🔵 CSA-D2 — per-constraint-family proptest sweep

M-S6 baseline = 5 active property tests (4 existing LogUp-side
+ 1 new CSA-S6 demo). Comprehensive sweep would add one
proptest variant per constraint family (~10 high-value
families × 8 cases × ~10 s per case = ~13 min added CI runtime).
Effort: 1 day.

**Disposition.** Deferred-as-deepening. Existing 5 + S4
deterministic coverage empirically validate 1.0 rejection
rate; deeper proptest sweep refines empirical confidence but
does not change the soundness verdict. See
`2026-05-20_CSA_S6_PROPERTY_BASED_TESTS.md` § 4.

### Cross-link

Per-AIR sign-off + the CSA stage rollup:
`2026-05-20_CSA_S7_AUDIT_SIGNOFF.md` §§ 1, 5.
**No new soundness findings; only deepening items.**
