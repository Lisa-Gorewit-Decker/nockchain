...
| **B1.1 (2nd model)** | no¹ | ✅ DONE | **Corroborated on a second, architecturally-different real Pearl model:** `Gemma-4-31B-it-pearl` (gemma4, hidden=5376/intermediate=21504, single-file 31 GB, same `quant_method:"pearl"` group split). `b1_1_gemma_{a,b,c}` mirror a/b/c at Gemma's real μ (k=5376). `pearl_model_compat` 11/0/0. `db3e193` |
